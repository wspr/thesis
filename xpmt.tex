...
Velocity sensors are used for feedback control to add damping to the system (see \secref[vref]{skyhook}).
...
The velocity of an object may be estimated by using a distance sensor from a known location and differentiating the output; in a vibration isolation setting, this allows to be measured the relative velocity between the ground disturbance and the resulting excitation ($\VelMass-\VelBase$ in terms of \figref{simple-isolation}).

While the angular velocity of an object with reference to the `fixed' earth can be directly infered by measuring the resulting coriolis force induced on a vibrating element \cite{konno1996}, the translational velocity cannot be measured directly.
Translation velocity can be estimated by integrating a measured acceleration, however; this technique (which is used in the experimental apparatus) is discussed in the next section.

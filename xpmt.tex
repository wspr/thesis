...
and capacitive sensors \cite{nijsse2001} may be used to measure relative velocity directly, while relative velocity may be estimated by differentiating the output of a displacement sensor. Such estimation is prone to high frequency noise that must be filtered away.

While the angular velocity of an object with reference to the `fixed' earth can be directly infered by measuring the resulting coriolis force induced on a vibrating element \cite{konno1996}, the absolute translational velocity cannot be measured directly.
Absolute translation velocity ($\VelMass$) can be estimated by integrating a measured acceleration, however; this technique (which is used in the experimental apparatus) is discussed in the next section.

...
\referpaper{The work presented in this chapter has been submitted in condensed form as a conference paper~\cite{robertson2013-aas-qzs}.}
...
The experimental apparatus that was designed and built as part of this study is shown as a schematic in \figref{rig} and as a photograph in \figref{rig-photo}.
...
\tabref{rigprop}. The experimental apparatus was built to serve as a platform for
...
the concepts underpinning the \qzs/ arrangement analysed in detail in \secref{qzs}.

\begin{figure}
  \scalebox{0.8}{\asyinclude{\jobname/rig-schematic}}
  ...
  \hspace*{1cm}\includegraphics[scale=0.8]{PhD/Experiments/Rig/photo-labelled.pdf}
...
\begin{figure}
  ...
  \lofcaption{Schematic of the dual-coil electromagnet built for the apparatus.}{ (Not to scale.)}
  \figlabel{rig-dual-coil}
  \end{sidefigure}\hfil
  \begin{sidefigure}
  \psfragfig{\phdpath Simulations/Coil/fig/2coils-normdesign}
  \lofcaption{Normalised force \vs\ displacement curve for the electromagnet.\figlabel{2coils-normdesign}}
...
A normalised force \vs\  displacement curve of this particuar coil is shown in \figref{2coils-normdesign}; from this
...
Velocity sensors can be used for feedback control to add damping to the system (see \secref[vref]{skyhook-intro}) in order to aid the vibration isolation characteristic of the system.
...
With a sensor attached to another body, separate from the object, relative velocity can be measured between the disturbance and the resulting excitation ($\gp{\velMass-\velBase}$ in terms of \figref{simple-isolation}).
...
Recently, \textcite{williams2009} showed a filter system designed to estimate absolute velocity from accelerometer measurements that can mitigate the low-frequency resonance issues seen due the high-pass filters present in the charge amplifiers used (seen in the experimental results in \secref*{acc-resonance}).
...
As will be shown (\secref*{cl-results}), the control methodology used for the results later in this chapter was indeed successful; it is expected that the method of \textcite{williams2009} may further improve the performance at low frequencies.
...
\begin{align}
\MagCentre_1 &= \OriginMagLow - \HeightBuffer - \half\HeightMag  \,, \\% base magnet
\MagCentre_2 &= \HeightRig - \gp{\OriginMagHigh - \HeightBuffer - \half\HeightMag}  \,, \\% upper magnet
\MagCentre_3 &= \DisplMag - \half\HeightRigMagnets + \half\HeightMag  \,, \\% B moving magnet
\MagCentre_4 &= \DisplMag + \half\HeightRigMagnets - \half\HeightMag  \,. % U moving magnet
\end{align}
...
The expected magnetic forces $F$ due to the measured displacements were calculated using the theory for coaxial cylindrical magnets (\secref[vref]{cyl-forces}) based on magnet centre displacements $\MagCentre_3-\MagCentre_1$ and $\MagCentre_2-\MagCentre_4$.
...
An alternate `nonlinear transmissibility' analysis of the measurements is performed in \secref{xpmt-nonlin}.

\begin{figure}
  \begin{wide}
  \begin{subfigure}
  \hspace*{-1cm}\psfragfig{PhD/Experiments/XPMT/latex/ol-undamped-frflin}
  \caption{Open circuit coil; no additional damping is added to the system.\figlabel{ol-undamped-frflin}}
  \end{subfigure}
  \qquad\qquad
  \begin{subfigure}
  \hspace*{-1cm}\psfragfig{PhD/Experiments/XPMT/latex/ol-damped-frflin}
  \caption
  {Closed circuit coil.
...
Plots of the curve fit models shown against the measured data are shown in \figref{ol-undamped-frflin-fit,ol-damped-frflin-fit}.
...
The natural frequencies and damping ratios calculated from this curve fitting are shown in \figref{ol-results}.
Comparing the results with the coil circuit open and closed, the resonance frequencies remained constant (\figref*{ol-reson}) but the damping ratios changed (\figref*{ol-dampi}).
...
\begin{figure}[p]
  ...
  \caption[Curve fit model of measurements with actuator disconnected.]{Curve fit model of measurements with actuator disconnected, \figref{ol-undamped-frflin}. Gray lines show the original data.}
  \figlabel{ol-undamped-frflin-fit}
\end{figure}

\begin{figure}[p]
  \psfragfig{PhD/Experiments/XPMT/latex/ol-damped-frflin-fit}
  \caption[Curve fit model of measurements with actuator connected]{Curve fit model of measurements with actuator connected, \figref{ol-damped-frflin}. Gray lines show the original data.}
...
\begin{figure}
  ...
  \begin{subfigure}
    ...
    \caption{Model-derived damping ratios.\figlabel{ol-dampi}}
  \end{subfigure}
  \end{wide}
...
the magnets through greater ranges of stiffness variation.
When the transmissibility is used to examine the broadband behaviour, the system is assumed to be linear and nonlinearities are rejected by the ratio of the cross-spectrum and power-spectrum terms in \eqref[vref]{Tbm}.
An alternative to transmissibility that does not reject nonlinearities can be calculated using the ratio of the individual power spectra of the magnet and base:
\begin{dmath}[label=frfnl]
  \transmissibility\fn{\freq} = \sqrt{\frac{\powerSpectMagnet}{\powerSpectBase}}
\end{dmath},
where $\powerSpectMagnet$ and $\powerSpectBase$ are the power spectral densities of the output magnet vibration and input base disturbance, \resp.
In this case, any nonlinearities in the signals are retained in the final result, and this calculation is referred to as the `nonlinear transmissibility' in this section.
(Recall a similar idea in \secref{qzs-not-zerk} comparing transmissibility with the variance gain.)

The nonlinear transmissibility for the coil in an open circuit (\ie, low damping) is shown in \figref{ol-undamped-frfnl}.
...
This superharmonic behaviour is similar to that seen in previous simulations in this thesis of such a \qzs/ system (\figref[vref]{variance-gaps}).
When the coil is connected and the damping present in the system increased, these nonlinearities are no longer apparent (the results are indistinguishable to those shown in \figref{ol-damped-frflin}).
...
\begin{figure}
  ...
  \lofcaption{Closed loop frequency response measurements for a range of velocity feedback gains.}{ The maximum feedback gain is close to instability for the system.}
  \figlabel{cl-results}
\end{figure}
...
Because the accelerometers were being used to measure velocity (via integrators in the charge amplifiers), the transmissibility curves shown in \figref{cl-results} were calculated from the
...
A metric of improvement to the vibration isolation characteristics of the system can be shown by calculating the \RSS/ transmissibility over a certain frequency range (recall \eqref[vref]{rsstrans}):
\begin{dmath}
  \inttrans = \sqrt{\Sum{\Abs{\transmissibility\fn{\freq}}^2}{\freq,\freq_1,\freq_2}}
\end{dmath}.
For the experimental results reported here, the lower frequency limit is defined as $\freq_1=\SI{1}{Hz}$, as the results become very noisy below this frequency.
The upper frequency limit $\freq_2=\SI{11}{Hz}$ was chosen as the higher-order dynamics (not shown in \figref{cl-results}) have little impact below this frequency.

\Figref{cl-reduction} shows the reduction in \RSS/ transmissibility
...
The resonance at \SI{1}{Hz} observed in \figref{cl-results} as the feedback gain increases causes the overall transmissibility reduction to have a local maximum.
...
Comparing the simulated results of \figref{cl-filter} with the experimental results of \figref{cl-results} shows a clear similarity between the two.

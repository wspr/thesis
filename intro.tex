...
\subsection{Examples of actual vibration isolation platforms}
...
\begin{figure}
   \asyinclude{PhD/Figures/Systems/vibration-inertial}
...
To show the affect of varying the resonance frequency of the \vibneut/, \figref{tuned-mass-vs-fig} shows the frequency response of a \vibneut/ with a range of neutraliser stiffnesses.
...
In this thesis, curve-fitting of magnetic force versus displacement characteristics has been used \emph{a priori} to establish such unknown parameters (\secref{qzs-mag}).
...
Vibration isolation systems designed using magnetic actuators
...
A system with similar characteristics is proposed in this work (\secref{oblique}).
...
An alternate design is shown by \textcite{hol2006}, which uses an axial bearing with \ang{90} rotated magnetisations to bear load in the vertical direction; the force--displacement characteristic is basically a mirror image to the attraction--repulsion pair.
Systems that use the negative stiffness between attracting magnets cannot be brought to a stable \qzs/ region due to their `softening spring' characteristic.
...
The variance gain of the system for $s$ and $x$ is shown in \figref{cubic-resonance-disturb,cubic-resonance-stiffness}, where $s$ is a white noise signal of variance $S$.
(The transmissibility is not examined because it removes .)
...
Significant nonlinearities in the response can be seen in both simulations, noting in \figref{cubic-resonance-disturb} that the resonance peak changes frequency with input amplitude.

\begin{figure}
  \includegraphics{PhD/Simulations/Zero_stiffness/eps/cubic-resonance-disturb}
...
\begin{figure}
  \includegraphics{PhD/Simulations/Zero_stiffness/eps/cubic-resonance-stiffness}
  \caption
  [Cubic stiffness response with varying values of the
...
\begin{itemize}
\item[\secref{unstable-vertical}]
\code{examples/mag\_ratio.m}.

\item[\secref{oblique}]
The scripts in \code{examples/oblique/}.

\item[\chapref{multipole}]
\code{examples/multipole\_example.m}

\item[\secref{optim-halbach-1d}]
The scripts in \code{examples/magspring/}.

\item[\secref{planar-multipole}]
\code{examples/planar\_multipole\_example.m} and \code{examples/planar\_compare.m}.

\item[\secref{magnetcoil-forces,magnetcoil-optimisation}]
This work is presented in the Mathematica code
\code{examples/Thick-Coil-Magnet-Forces.nb}
based on the general \code{MagnetCode.m} library.
\end{itemize}

Incidentally, the source for this thesis itself, including some of the programmatically drawn figures, is available in\\

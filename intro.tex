...
It is clear in the ideal case that skyhook damping is the more effective at reducing the total vibration of a system.
...
The general limits of control in practical applications was investigated by \textcite{ananthaganeshan2001,brennan2007-jsv} in a single \dof/ vibration suppression system, where time delays and phase lags introduced by digital filters
...
As pithily stated by \textcite{williams2009}:
\begin{quote}
...
\subsection{Further discussion of skyhook `damping'}
\seclabel{skyhook}
...
In the context of isolating vibration of a train, \textcite{li1999} discuss the disadvantages in tracking when applying skyhook damping for systems that experience very low frequency (\ie, steady state) changes in the base position, but for the purposes of vibration isolation this is not a problem for this work if the ground is assumed to experience disturbance with zero mean.
...
Sensor fusion for obtaining a useful estimate (for active control) of absolute velocity to arbitrarily low frequencies is an open research question.

\subsubsection{Semi-active skyhook damping}

The `skyhook' term commonly appears in the related field of semi-active vibration control, which uses dynamic stiffness control as a feedback mechanism to approximate absolute velocity feedback control \cite{leavitt2007}.
...
\subsection{Alternative control approaches}
...
More advanced methods are briefly discussed here to highlight the broad scope of the field of active vibration control and to give a brief taste of such variety.
These various methods are introduced below in approximate order of complexity and/or effectiveness.
...
The addition of inertial actuators produces additional resonances into the structure and the resonance of the actuator limits the broadband performance of the control. As such, this approach is best suited for use in \emph{a posteriori} vibration control measures.
...
In the most simple of these bearings, this instability is constrained with a physical stop.
...
\textcite{yonnet1981} highlighted in an early treatment on the topic that, even moreso than for magnetic bearings, periodic recurring magnetisation (see \secref{multipole}) is required for magnetic couplings to transmit torque satisfactorily.
...
One way to overcome this issue is to develop a low-order empirical model for the system, with unknown constants that must be identified; an example of this was shown for a coil-iron suspension by \textcite{agamennoni2004}.
In contrast, in this work where an empirical model has been used, curve-fitting has been used \emph{a priori} to establish the unknown parameters (\secref{qzs-mag}).
...
Sufficiently advanced nonlinear control can achieve stability and tracking without the use of system identification.
...
Experimental results of a magnetically levitated mass (\fxfatal{crossref}) later show that the damping in this case is very low, making a passive non-contact eddy current damper of potential importance for vibration suppression in systems that require additional passive damping.
...
(The transmissibility is not examined because it removes nonlinear components of the original signals.)
...
For completeness, this section covers the opposite of \qzs/: quasi--\emph{infinite} stiffness, largely expounded in a swath of papers over the last decade
...
Incidentally, the source for this thesis itself, including some of the programmatically drawn figures, is available in $\langle$\url{http://www.github.com/wspr/thesis}$\rangle$.
...
\item [\chapref{multipole}]
      A quick discussion of the use of `multipole' or `Halbach' arrays composed of many various-facing magnets for improving the load bearing capabilities for magnetic supports.
\item [\chapref{qzs}]

...
To illustrate the effect of various control gains, the transfer function \eqref{tf-genfeedback} is drawn with parameters $\mass=\SI{1}{kg}$, $\damping=\SI{0.5}{kg/s}$ and $\stiffness=\SI{10}{N/m}$, and each of the six control gains $\gainArbitrary$ varied independently.

\paragraph{Displacement feedback}
...
As discussed later \fixme{crossref}, this form of feedback is highly susceptable to low frequency instabilities and cannot reliably be implemented in practice.

The system with displacement feedback is stable according the inequality
\begin{dmath}
  \damping>\Re\fn{\sqrt{\damping^2-4 \gp{\gainDisp+\gainSkyspring+\stiffness} \mass}}
\end{dmath},
...
\begin{dmath}
  \Re\fn{\frac{\damping \pm \sqrt{\damping^2-4 \stiffness \gp{\gainAcc+\gainSkymass+\mass}}}{\gainAcc+\gainSkymass+\mass}}>0
\end{dmath},
...
\begin{dmath}
  \damping+\gainSkyhook+\gainVel \pm 
    \Re\fn{\sqrt{\gp{\damping+\gainSkyhook+\gainVel}^2-4 \stiffness \mass}}>0
\end{dmath},
...
The absolute and relative velocity feedback results may be compared by calculating the \RMS\ transmissibilities over a frequency range of interest ($\sqrt{\Int{\transmissibility\fn\freq}{\freq,\freq_1,\freq_2}}$) as a function of increasing feedback gain for the two cases. 
...
\paragraph{Interaction with passive control}
...
This becomes a major theme of the thesis, explored later in this introduction in \secref{qzs-explore}.
...
Semi-active stiffness modification is used more commonly in \vibneut/ applications, described later in \secref{vibneut}, since for vibration isolation applications in general the lower the stiffness the better.
...
\subsubsection{Higher-level control}
...
\textcite{savaresi2007} implement a combination skyhook/acceleration-feedback controller in the context of vehicle vibration control for ride `smoothness'; the premise is that skyhook damping knocks out resonance peaks (without any other side-effects; \cf\ relative velocity feedback) while absolute acceleration feedback decreases the overall transmissibility at higher frequencies.
...
Here, some minor digressions are made to place the rest of the literature in context with alternate approaches, and some discussion made on why the techniques herein were deemed unsuitable for the work of this thesis.
...
\textcite{benassi2002-part2} showed in experiment that this can be improved by the addition of a phase lag controller with force feedback in conjuection with velocity feedback.

...
\begin{figure}[!b]
  ...
  \lofcaption{Two main types of vibration control problem.}
  {
    In both cases, mass $\mass$ is being excited and has dynamics according to linear stiffness $\stiffness$ and damping $\damping$ parameters. Input control force $\forceIn$ can be generated using feedforward or feedback control to minimise the displacement $\dispMass$ of the mass.
  }
\end{figure}

The most basic vibratory or oscillating system is shown in \figref{simple-suppression}, in which a mass is excited by an external source and behaves with resultant dynamics determined by the linear stiffness and damping of the connection.
...
Introduced in the previous section, \figref{simple-isolation} shows the simplest form of vibration isolation in which a mass $\mass$ is mounted with stiffness $\stiffnessRel$ and damping $\damping$, and with disturbance input $\dispBase$.
...
\begin{figure}[p]
   ...
   \begin{subfigure}
     ...
     \caption[Absolute displacement feedback.]{Absolute displacement feedback, $\gainSkyspring$.}
   \end{subfigure}
   \end{wide}
   ...
   \begin{subfigure}
     ...
     \caption[Absolute acceleration feedback.]{Absolute acceleration feedback, $\gainSkymass$.}
   \end{subfigure}
   \end{wide}
...
The absolute and relative velocity feedback results may be compared by calculating the \RSS/ transmissibilities $\inttrans$ over a frequency range of interest as a function of increasing feedback gain for the two cases.
The \RSS/ transmissibility is calculated as
\begin{equation}\eqlabel{rsstrans}
\inttrans=\sqrt{\Sum{\Abs{\transmissibility\fn{\ii\freq}}^2}{\freq,\freq_1,\freq_2}}\,,
\end{equation}
and is of interest as it gives a relative measure of the total energy attenuation through the system.
The comparison of \RSS/ transmissibilities is shown in \figref{rms-transmissibility}, where the relative feedback case has a local minimum whereas in the absolute feedback case it continuously decreases.
...
\begin{figure}
   \psfragfig{\jobname/rms-transmissibility}
   \caption[Root-sum-square transmissibility versus feedback gain of relative and
   absolute (`skyhook') velocity feedback control.]{
   Root-sum-square transmissibility versus feedback gain of relative and
   absolute (`skyhook') velocity feedback control of a linear vibration isolation system (\figref*{simple-isolation}).}
...
Semi-active skyhook damping control was introduced by \textcite{karnopp1974-jei}, and it is often used in vehicle suspension where active systems can be impractical due to weight and power constraints (which is not always the case \parencite{gysen2008-vppc}).
...
In the original method of implementing skyhook control in this manner, the switching mode that is used to emulate the `skyhook' in the controlled damping introduces higher order harmonics in the frequency response, as shown by \textcite{ahmadian2001}, who later proposed two `jerk-free' skyhook algorithms to avoid this problem \cite{ahmadian2004}.
...
For inertial actuators, velocity feedback can have problems with stability margins at low frequencies.
...
The addition of inertial actuators produces additional resonances into the structure and the resonance of the actuator limits the broadband performance of the control. As such, this approach is best suited for use in \emph{a posteriori} vibration control measures or in narrowband vibration control as discussed next in more general terms.

\subsubsection{Narrow-band vibration control}
\seclabel{vibneut}
...
To show the effect of varying the resonance frequency of the \vibneut/, \figref{tuned-mass-vs-fig} shows the transfer function $\laplaceMass/\laplaceBase$ of a \vibneut/ (as defined in \figref{vibration-absorb}) with a range of neutraliser stiffnesses with other parameters as shown in \tabref{vibneut-param}.
...
\begin{figure}
   \psfragfig{\jobname/tuned-mass-vs-freq}
   \caption[
     Transmissibility of a structure with a \vibneut/.
   ]{
     Transmissibility of a structure with a \vibneut/ for a range of neutraliser stiffnesses $\stiffnessAbsorb$ (labelled).
...
\begin{table}
\caption[Simulation parameters for the \vibneut/ results.]{Simulation parameters for the \vibneut/ results in \figref{tuned-mass-vs-fig}.}
...
Conversely, if the \RSS/ transmissibility of an entire frequency band is calculated (\figref*{rms-inertial}) it can be seen that lower absorber damping \emph{decreases} the overall vibration reduction.
...
\begin{wide}
  ...
  \hfil
  \begin{subfigure}[0.48]
    \psfragfig{\jobname/rms-inertial}
    \caption{
      Root-sum-square transmissibility \vs\ stiffness.
      Greater broadband reductions result from \emph{higher} damping.\figlabel{rms-inertial}}
  \end{subfigure}
\end{wide}
\caption[Single-frequency transmissibility reduction and broadband \RSS/ transmissibility versus \vibneut/ stiffness.]{
  Single-frequency transmissibility reduction and broadband \RSS/ transmissibility versus \vibneut/ stiffness for a range of absorber damping ratios.
}
\end{figure}
...
Earnshaw's theorem for stability (\secref*{earnshaw}) is not applicable for \maglev/ systems, at least while the vehicle is in motion, since the motion of the vehicle adds a time-varying element to the magnetic system.
When at rest, \maglev/ vehicles tend to be anchored to the tracks for stability.
...
\note{Note that while these six \dof/ actuators have very high static precision, they do not have the frequency response of the so-called `nano-positioners' using piezoelectric stack actuators.}
...
\end{itemize}
The electromagnetic design principles of these devices are based around actuation rather than supporting a load.
...
\textcite{yonnet1981} highlighted in an early treatment on the topic that, even more so than for magnetic bearings, periodic recurring magnetisation (\figref*{backers-bearing}) is required for magnetic couplings to transmit torque satisfactorily.
...
\begin{figure}
  \includegraphics[width=0.5\textwidth]{\jobname/saddle}
...
\textcite{dangola2006} analysed the dynamics of a nonlinear system in which a variation of both stiffness and mass by up to 50\% resulted in a change in resonance frequency of less than 6\%.
...
\end{dmath},
where $\crossSpectMagnetBase$ is the cross power spectral density between the input and output signals and $\powerSpectBase$ is the power spectral density of the input signal \parencite{bendat1993-book}.
...
can be calculated as a ratio of the \RMS/ output to input signals:
...
\begin{figure}
  \begin{wide}
    \psfragfig{PhD/Simulations/Zero_stiffness/fig/cubic-resonance-disturb-a}
    \hspace*{+0.5cm}
    \psfragfig{PhD/Simulations/Zero_stiffness/fig/cubic-resonance-disturb-c}
    \hspace*{-1.5cm}
  \end{wide}
...
\begin{figure}
  \begin{wide}
    ...
    \hspace*{+0.5cm}
    \psfragfig{PhD/Simulations/Zero_stiffness/fig/cubic-resonance-disturb-d}
    \hspace*{-1.5cm}
  \end{wide}
...
\begin{table}
\caption{Parameters used to simulate the dynamics of \eqref{duffing} (\figref*{cubic-resonance-disturb}).}
...
\citeconfx{robertson2013-aas-qzs}
\citeconfx{robertson2013-aas-zks}
\end{itemize}
\endgroup

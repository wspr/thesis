...
\begin{subfigure}
  ...
  \caption{Inclined springs in their unloaded, uncompressed state, corresponding to a vertical displacement of $\ZKSdisp=\ZKSheight$.\figlabel{zks-dia}}
\end{subfigure}
\begin{subfigure}
  \asyfig{ZKS/schematic-qzs}
  \caption{Inclined springs at a position of maximum negative stiffness, corresponding to a vertical displacement of $\ZKSdisp=0$.\figlabel{zks-qzs}}
\end{subfigure}
\end{wide}
...
\begin{table}
  \caption{Properties of the springs in the \qzs\ inclined spring system defining stiffness ratio $\ZKSkratio$ and length ratio $\ZKSlengthratio$.}
...
=  \ZKSdisp \ZKSstiffincl \gp{\frac{\sqrt{\ZKSheight^2+\ZKSwidth^2}}{\sqrt{\ZKSwidth^2+\ZKSdisp^2}}-1} 
\end{dmath}.
...
\begin{dmath}[label=ZKSforceinclVnorm]
  \frac{\ZKSforceinclV{\ZKSnorm}}{\ZKSheight \ZKSstiffincl} = \ZKSnorm \gp{\sqrt{\frac{\ZKSratio^2+1}{\ZKSratio^2+\ZKSnorm^2}}-1}
\end{dmath}.
...
\begin{wide}
\begin{subfigure}
\psfragfig{\phdpath Simulations/Springs/fig/ZKSforce-plain}
...
\end{subfigure}
\begin{subfigure}
\psfragfig{\phdpath Simulations/Springs/fig/ZKSforce-total}
...
\end{subfigure}
\end{wide}
\caption{Vertical forces due to the inclined springs. $\ZKSratioQZS$ is the value of $\ZKSratio$ for which \qzs\ is achieved at $\ZKSnorm$, calculated from \eqref{ZKSratioQZS}.}
...
\frac{\ZKSforcetotalV}{\ZKSheight \ZKSstiffincl} = -\ZKSnorm \ZKSkratio+\ZKSkratio+2 \ZKSnorm \gp{\sqrt{\frac{\ZKSratio^2+1}{\ZKSratio^2+\ZKSnorm^2}}-1}
\end{dmath},
...
\begin{wide}
\begin{subfigure}
\psfragfig{\phdpath Simulations/Springs/fig/ZKSstiffness-total}
...
\end{subfigure}
\begin{subfigure}
\psfragfig{\phdpath Simulations/Springs/fig/ZKSstiffnessH-total}
...
\begin{wide}
\begin{subfigure}
  \psfragfig{\phdpath Simulations/Springs/fig/ZKSoffqzs}
  \caption{\figlabel{ZKSoff}
The stable and unstable equilibrium points for $\ZKSoff\in\{-0.1, 0, 0.1\}$. The rest position will move from the unstable point to the stable point of equilibrium.}
\end{subfigure}
\begin{subfigure}
  \psfragfig{\phdpath Simulations/Springs/fig/ZKSratioErr}
  \caption{\figlabel{ZKS-ratioErr}
The stiffness at equilibrium as $\ZKSoff$ varies; as the stiffness becomes negative, the stiffness shown corresponds to the stable point of equilibrium shown in the figure adjacent.}
\end{subfigure}
\end{wide}
\caption{Force and stiffness of the inclined spring system near \qzs, showing the effect of unstable equilibrium.}
\end{figure}
...
\begin{figure}
\psfragfig{\phdpath Simulations/Springs/fig/ZKSkratio-effect}
...
\begin{figure}
  \psfragfig{\phdpath Simulations/Springs/fig/ZKS-hk-ratio}
  \caption{Relationship between $\ZKSkratio$ and $\ZKSlengthratio$ at \qzs\ both horizontally and vertically.}
...
\begin{wide}
\begin{subfigure}
  \psfragfig{\phdpath Simulations/Springs/fig/ZKSstiffnessV-QZS}
  \caption{\figlabel{ZKSstiffnessV-QZS} Normalised vertical stiffness of the system.}
\end{subfigure}
\begin{subfigure}
  \psfragfig{\phdpath Simulations/Springs/fig/ZKSstiffnessH-QZS}
  \caption{\figlabel{ZKSstiffnessH-QZS} Normalised horizontal stiffness of the system.}
\end{subfigure}
\end{wide}
\caption{A comparison of vertical and horizontal stiffness characteristics at \qzs\ in both directions, for a range of spring length ratios, $\ZKSlengthratio$.}
\figlabel{ZKSstiffness-bothQZS}
\end{figure}

\begin{figure}[p]
  \psfragfig{\phdpath Simulations/Springs/fig/ZKSstiffnessH-NotQZS}
  \caption{Normalised horizontal stiffness of the system at $\ZKSkratio=0.95\ZKSkratioQZS$ in order to obtain a small range of displacement around $\ZKSnorm=0$ with positive stiffness (compare with \figref{ZKSstiffnessH-QZS} which has negative stiffness for $\ZKSnorm<0$). The vertical \qzs\ condition is unaffected.}
...
\begin{figure}
\psfragfig{\phdpath Simulations/Springs/fig/ZKScompress}
...
\begin{figure}
  ...
  \caption{Schematic of a magnetic spring with quasi-zero stiffness at $\ndispZ=0$ to isolate displacement $x$ from disturbance vibration $y$. Large arrows indicate direction of polarisation of the magnets. In this paper, cubical magnets are used with side length $\mdim$, distance $2\mdim\ngap$ between the centres of the fixed magnets, and displacement of the floating magnet from the zero stiffness position $\mdim\ndispZ$. Rest position at positive $\ndispZ$ corresponds to unstable equilibrium; negative $\ndispZ$ is stable.}
  \figlabel{schem}
\end{figure}
...
quadratic-type spring is marginally unstable at its zero stiffness
...
The structure of the paper is as follows. The exact equation for the
...
constraints that govern the behaviour of a vibration isolation device
are applied to the \qzs\ magnetic spring and suitable ranges for the
design parameters are found to achieve the design goals.
\fixme{not happy with the previous sentence.}
...
analytically with the formulation of \textcite{akoun1984}, for magnet
...
\begin{figure}
  ...
  magnetisations in the $z$ direction, distance between their centres
...
\begin{dmath}
  \ndisp = \gamma/\mdim .
\end{dmath}
After simplification, the force $\magforce$ on the second magnet in
attraction (\ie, for magnets with polarisation in the same direction)
can be written as a function of magnet dimension, $\mdim$, and
normalised magnet displacement, $\ndisp$:
\begin{dmath}[label=force]
  \magforce = \mdim^2 \nforce .
\end{dmath}
For magnets in repulsion, the expression has opposite sign.  The force
is directly proportional to the facing area of the magnets, $\mdim^2$,
for a fixed normalised displacement, $\ndisp$. The derived analytical
expression for the normalised force $\nforce$ is given in
\eqref{nforce} in the appendix. 

The stiffness between two cubical magnets can be calculated by differentiating
the force expression in \eqref{force} with respect to displacement
$x=\mdim\ndisp$ and can be shown to be proportional to the magnet size
$\mdim$:
\begin{dmath}[label=stiffness]
  \magstiffness = \frac{\partial}{\partial x} \magforce
             = \frac1\mdim\frac{\partial}{\partial\ndisp} \mdim^2\nforce
             = \mdim\nstiffness .
\end{dmath}
The derived analytical expression for the normalised stiffness $\nstiffness$
is given in \eqref{nstiffness} in the appendix.

A `zero stiffness' magnetic spring consists of an attracting magnetic
pair above a repelling magnet pair as shown in
\figref{schem}. Parameter $\mdim \ngap$ is the gap between the centres
of the magnet pairs at zero stiffness, and $\mdim \ndispZ$ is the
static displacement of the floating mass about the centre line between
the magnets. The force due to the lower magnet in repulsion is
\begin{align}
...
The force $\nforceZ$ and stiffness $\nstiffnessZ$ of the \qzs\ spring are
...
the resulting model is dependent on the gap beteen the magnets and local to
...
variation sufficently well.

Due to the complexity of the analytical expression, a simpler approximation of
...
Using \eqref{nforceZ} as the exact behaviour of the \qzs\ system, (without
loss of generality, the magnetisation of each magnet, $J$, is taken as unity),
and taking a least squares fit with \eqref{nnfit} varying all three parameters
$\ccA$, $\ccB$, and $\nn$ over the range $2\le\ndispZ\le5$, modelling errors
of less than $1\%$ are achieved. Including displacements outside of this range
...
\begin{figure}
  ...
  \caption{Approximation errors of \eqref{nnfit} compared to the
    analytical solution, for integer and real values of $\nn$.}
  \figlabel{nnfit}
\end{figure}
...
\begin{table}
  ...
  \caption{%
    ...
    are empirical, and unitless, parameters.}
  \tablabel{nnfit}
\end{table}

A very accurate approximation for the force in Newtons between two
cubical magnets is thus given by
\begin{dmath}
\magforce \approx \pm a^2J^2\frac{\SI{6.028e5}{}}{\gp{0.1883+al}^{4.197}}
\end{dmath}
for magnet size, $a$, and displacement, $al$, both in metres, and
magnetisation, $J$, in Tesla, where the expression is positive for
magnets in repulsion and negative for magnets in attraction.

\Figref{nnforceZ} shows the model of
the \qzs\ system using \eqref{nnfit} to calculate the force due to the
...
\begin{figure}  {\psfragfig{\phdpath QZS/fig/nnfit-z}}
  \caption{Normalised force $\nforceZ$ vs.\ displacement $\ndispZ$
    curves of a quasi-zero stiffness magnetic system for a range of
    normalised gap $\ngap$. Single points correspond to the exact
...
In the previous section, a \qzs\ magnetic system was introduced with an exact
expression and various approximations for calculating the force/displacement
curves for a range of system designs. It is proposed that this system is
...
parameters to satisfy this criterion and a design principle developed.
...
\eqref{eq} must be solved subject to this constraint. \Figref{heq-map}
...
spring. The `strong' constraint is defined to achieve these criteria:
\begin{dmath}
\nforceZ{1,0} < \mass g/\mdim^2 < \nforceZ{1,-\ngap+1}, \eqlabel{strong}
\end{dmath}
which is shown graphically in \figref{heq}.  This constraint is
depicted in \figref{cons-strong} as a region over the parameters
$\mdim$ and $\ngap$. The general trend follows intuitively: for a
fixed magnet gap, larger magnets are required to support larger loads.

\begin{figure}
  \asyfig{Magnets/heq-combined}
  \caption{Normalised force vs.\ normalised displacement curve illustrating the bounds of both allowable load that can be bourne and the displacement range of the magnet; see \eqref{strong,stable} respectively for the mathematical representations. $-\ngap+1$ is the position where the floating magnet is touching the upper face of the lower fixed magnet, and $-\epsilon$ is the closest allowable position to the unstable zero stiffess position.}
  \figlabel{heq}
\end{figure}
...
\begin{figure}
  ...
  \caption{Regions of $\mdim$ and $\ngap$ satisfying the constraint
    `strong' of \eqref{strong} for a range of masses. 
...
$\keq{}$, must satisfy the resonance frequency constraint:
...
\end{dmath}
This constraint is shown for a range of desired nominal stiffnesses in
...
\begin{figure}
  ...
  \caption{Regions of $\mdim$ and $\ngap$ satisfying the constraint
    `soft' of \eqref{soft} for a range of resonance frequencies and a
...
\end{dmath}
where $\mdim \epsilon$ is the tolerance of the closest allowable
distance to the (marginally unstable) zero stiffness position that the
system can operate. The `stable' constraint of \eqref{stable} ensures
that the magnet is not perturbed past the \qzs\ position into the
unstable region. The `saturate' constraint of \eqref{saturate} ensures
that the lower displacement bound lies above the face of the fixed
lower magnet.

These latter contraints impose only small limits on the design of the
system. The `saturate' constraint, shown in \figref{cons-saturate} as
...
\begin{figure}
  ...
  \caption{Regions of $\mdim$ and $\ndispZ$ satisfying the constraint
    `saturate' of \eqref{saturate} for a range of disturbance
...
Note that the maximum displacement of the spring will not be symmetric with
input displacement due to the softening spring stiffness. Without loss of
generality, however, it is possible to represent the maximum displacement in
the `stable' and `saturate' constraints (\eqref{saturate,soft}) with the same
symbol.

The stability constraint is governed by two parameters, $\dmax$ and
$\epsilon$, which are varied in \figref{cons-stable-1,cons-stable-2}
respectively. Again, this constraint only has a small effect on the
contraint region, but it is an important effect. Because the stability
constraint limits the minimum distance from the equilibrium position,
this prevents the spring from reaching the very low equilibrium
stiffnesses found just below the zero stiffness position.
...
\begin{figure}
  ...
  \caption{Regions of $\mdim$ and $\ngap$ satisfying the constraint
    `stable' of \eqref{stable} for a range of normalised displacement
    tolerances $\epsilon$ and a maximum disturbance displacement of
    $\dmax=\SI{5}{mm}$. Darker sections denote overlap of the regions
    in the overlay plot.}
...
\begin{figure}
  ...
  \caption{Region of $\mdim$, $\ngap$ satisfying the constraint `stable'
...
The introduced constraints may be simultaneously satisfied for some specified
values of mass, resonance frequency, and displacement range ($\mass$, $\freqdesign$,
and $\dmax$) by varying the free parameters magnet size $\mdim$ and magnet gap
$\mdim\ngap$. The shared region of the `strong', `soft', `saturate', and
`stable' constraints (\eqref{strong,soft,saturate,stable}) are shown in
\figref{cons-all} for various combinations of supported mass and resonance
frequency. This figure presents a complete design map that shows how the
techniques presented in this paper can be used for speculative design work and
optimisation.
...
\begin{figure}
  ...
  \end{wide}
  \caption{Regions of $\mdim$, $\ngap$ satisfying all constraints,
    demonstrating the effects of changing the mass $\mass$ and the
    resonance frequency $\freqdesign$ of the desired system. Darker sections 
    denote overlap of the regions in the overlay plot}
  \figlabel{cons-all}
\end{figure}

As the cost of the magnets increases with volume, a final `small'
constraint can be imposed:
\begin{dmath}
  \min\fn{\mdim}
\end{dmath}.
The magnet size will typically be limited by availability of discrete
sizes from the manufacturer.

\subsection{Nonlinear behaviour}\seclabel{nonlin}
...
constraint. However, as the mass is perturbed from equilibrium the
...
spring at equilibrium over its maximum positive and negative
...
decrease. Comparing \figref{nl-contours} to \figref{heq-map}, it can also be
seen that the nonlinearity increases the closer the equilibrium displacement
becomes to the zero stiffness position.

\begin{figure}
  \psfragfig{\phdpath QZS/fig/nl-contourss}
  \caption{Contours of nonlinearity shown inside the region of $\mdim$,
    $h$ satisfying all constraints for parameters $\mass=\SI{1}{kg}$,
...
\begin{figure}
  ...
  \caption{Zoom of \figref{nl-contours} to show the behaviour of the nonlinearity in the lower limits of a magnetic \qzs\ design.}
...
$\mdim=\SI{20}{mm}$, $\ngap=3$, resonance frequency $\natfreq=\SI{3.58}{Hz}$,
and equilibrium position $\heq{}$ is \SI{10.4}{mm} below the zero stiffness
...
increases. Conversely, as the mass moves upwards, closer to the zero stiffness
position, the stiffness and force both decrease.

The results shown in \figref{nl} indicate that for small magnitude
disturbances the phase plot is very similar to a linear system. For the
requirements of high precision applications, this indicates that the
nonlinearity of the spring will not be detrimental to the performance of the
vibration isolation device.

\begin{figure}
  \psfragfig{\phdpath QZS/fig/nl}
  \caption{Phase plot of the \qzs\ system at steady
    state resonance for a range of input disturbance amplitudes. The
...
It is clear from \figref{nl} that for small vibration amplitudes the
nonlinearity is very small; the system has an equivalent response to the
linear system. As the amplitude increases, the nonlinear system response
increases at a slower rate than the linear spring. This slowdown is due to a
shifting of the resonance peak as the nonlinearity increases.

The `nonlinear transmissibility' of a nonlinear response, also known 
as `variance gain' \cite{savaresi2007}, can be calculated
as a ratio of the \textsc{rms} output to input signals:
\begin{dmath}[label=nl-trans]
T_{\text{nonlin}} = \sqrt{
  \tfrac1T\Int{\bar x\fn t^2}{t,0,T}/
  \tfrac1T\Int{\bar y\fn t^2}{t,0,T}
}
\end{dmath},
where $T$ is the time interval over which the transmissibility is calculated,
and $\bar x$ and $\bar y$ are the mean-zero output and input displacements of
the vibration isolator, respectively. For linear systems, this expression
collapses to the standard formulation for transmissibility. For nonlinear
systems, \eqref{nl-trans} describes the ratio of output to input energy for a
given excitation. For the tonal input disturbance of \eqref{y}, the nonlinear
transmissibility at the frequency of excitation is
\begin{dmath}[label=nl-trans2]
T_{\text{nonlin}} = \frac{\sqrt2}{A_e}\sqrt{
  \tfrac1T\Int{\bar x\fn t^2}{t,0,T}
}
\end{dmath}.

\Figref{nl-trans} shows the nonlinear transmissibility of the \qzs\ system for
a relatively large amplitude of excitation $A_e=\SI{1}{mm}$ which corresponds
to a large measure of nonlinearity (see \tabref{nl}), with system parameters
as described earlier in this section for \figref{nl}. At low amplitudes, the
nonlinear transmissibility approaches the linear system equivalent. At higher
amplitudes, the resonance peak can be seen to distort slightly into the lower
frequencies, but there is no effect on the response away from resonance.

\begin{figure}  
  \psfragfig{\phdpath QZS/fig/nl-trans}%
  \hfil
  \psfragfig{\phdpath QZS/fig/nl-trans-zoom}%
  \caption{Nonlinear transmissibility as given by \eqref{nl-trans2} at an 
  excitation amplitude of $A_e=\SI{1}{mm}$; the second plot is a zoom of the 
  resonance peak. The dashed lines are the nonlinear system; solid lines 
  represent the equivalent linear system. Even with a large excitation 
  amplitude the effect of the nonlinearity of the \qzs\ system is small.}
  \figlabel{nl-trans}
\end{figure}

It is important to consider the role of damping in the results shown in
\figref{nl}. A low damping coefficient results in a greater resonance response
in the low frequency range. This increase in the displacement response will
also increase the nonlinear behaviour of the spring. However, the advantage of
low damping is a very fast roll-off in vibration attenuation at frequencies
above resonance. Since the damping of the non-contact magnetic spring is very
low, either the bandwidth of excitation must lie above the resonance frequency
or active skyhook damping must be applied in order to reduce the strong
effect that the resonance has on the low-frequency response. As previously
discussed, skyhook damping is especially suitable for this purpose because
the isolation region of the frequency response is not affected.

\subsection{Summary}\seclabel{end}

This paper has analysed a \qzs\ magnetic spring for the purposes of load
bearing with low stiffness. Exact and approximate expressions were derived for
cube-shaped magnets for analysing the behaviour of this system. The
approximate expression is very simple and accurate over a large displacement
range, and may be used for cubical magnets of any size.

Four design constraints were imposed on the design in terms of the two
variable design parameters: magnet size and gap between the fixed magnets. A
technique for explicitly mapping these contraints to a map of valid parameters
was shown. Allowable stroke and magnet size availability allow a design to be
optimised using this technique based on the required load bearing and
resonance frequency.

The magnetic isolator is weakly nonlinear with a distorted phase plot compared
to a linear system; the nonlinear transmissiblity shows a resonance peak
skewed into the lower frequencies. These nonlinearities only become apparent
at large excitation amplitudes (that is, large measures of nonlinearity) and
are not significant enough to affect the vibration isolation properties of the
device.
...
design readily lends itself to performance enhancement via active means, such
as skyhook damping or reduction of the natural frequency via feedback control.

...
\referpaper{The work presented in \secref{qzs} is based on material that has been published as a journal paper~\cite{robertson2009-jsv}. The work in \secref{qzszks} has been submitted as a conference paper~\cite{robertson2013-aas-zks}.}
...
The first, in \secref{qzszks}, is an `inclined spring' device that has been studied previously in the literature; here, it is studied from a fresh perspective in order to contrast its behaviour with the second \qzs/ system examined in \secref{qzs}: an arrangement of attracting and repelling permanent magnets that allows simpler tuning of the system to adapt to dynamic load bearing.
...
\begin{wide}
\subbottom
  [Vertical force due to inclined springs only using
\eqref{ZKSforceinclV} for a range of geometric ratios
$\ZKSratio$.\figlabel{ZKSforce-plain}]
  {\hspace*{-1.5cm}\psfragfig{\jobname/ZKSforce-plain}}
\quad
\subbottom
  [Normalised vertical force characteristic of the system calculated
...
spring being compressed to zero length.\figlabel{ZKSforcetotalVnorm}]
  {\psfragfig{\jobname/ZKSforce-total}}
\end{wide}
...
\figref{ZKSforcetotalVnorm} for a unity stiffness ratio \mbox{($\ZKSkratio=1$)}, where it can be seen that by
...
\subbottom
  ...
  {\null\hspace*{-1cm}\psfragfig{\jobname/ZKSstiffness-total}}%
\quad
\subbottom
  [
  Normalised horizontal stiffness of the system calculated with \eqref{ZKSstiffnessH}.
  \figlabel{ZKSstiffness-horiz}
  ]{\psfragfig{\jobname/ZKSstiffnessH-total}}
\end{wide}
\caption[Vertical and horizontal stiffness characteristics for a range of geometric ratios.]
 {Vertical and horizontal stiffness characteristics for a range of geometric ratios $\ZKSratio$ at $\ZKSkratio=1$. Plots are labelled with their ratio to $\ZKSratioQZS$, which is calculated for a length ratio of $\ZKSlengthratio=1$.}
...
The graph of horizontal stiffness versus vertical displacement is shown in \figref{ZKSstiffnessH-QZS}.
...
Normalised vertical stiffness of the system.]
  {\psfragfig{\jobname/ZKSstiffnessV-QZS}}
\quad
...
negative displacement will result in negative stiffness.]
  {\psfragfig{\jobname/ZKSstiffnessH-QZS}}
\end{wide}
\caption
...
\begin{figure}[p]
  \psfragfig{\jobname/ZKSstiffnessH-NotQZS}
  \caption
...
The distance between the magnet centres $\cdz$ (\figref[vref]{akoun}) with respect to the size of the magnets (side length $\mdim$) is expressed as a normalised length $\ndisp$:
\begin{dmath}
  \ndisp = \cdz/\mdim \,.
\end{dmath}
...
\begin{table}[bp]
  \lofcaption{
    ...
    three parameters.
    Note that these constants are unitless.
  }
  \tablabel{nnfit}
...
\end{description}
\smallskip
Bounds of normalised force and normalised displacement are depicted graphically in \figref{heq} to help visualise these constraints, as will be discussed further as each criterion is introduced and defined mathematically.

\begin{figure}
  \hspace*{1.5cm}\asyinclude{\jobname/heq-combined}
  \caption
...
The first inequality in \eqref{strong} corresponds to the curved diagonal boundary in \figref{cons-strong} and the second inequality corresponds to the vertical boundary.
...
In \figrangeref{cons-strong}{cons-stable}, the individual criteria are plotted with a range of relevant parameters on three separate graphs.
...
For example, it can be seen in \figref{cons-strong} that adjusting the mass affects the region for both magnet size and magnet gap.
...
\figref{cons-soft}. As the natural frequency decreases, larger
...
force/displacement plot as shown the horizontal axis of \figref{heq}, which shows a maximum
...
With reference to \figref{heq}, this `saturation' criterion is
...
The saturation criterion, shown in \figref{cons-saturate} as a region over the parameters $\mdim$ and $\ngap$ for a range of $\dmax$, limits the lower size of the magnet.
...
\begin{figure}[htbp]
  \begin{wide}
  \subbottom[{A range of normalised displacement
  tolerances $\dmin$ and a maximum disturbance displacement of
  $\dmax=\SI{5}{mm}$.
  \figlabel{cons-stable-1}}]{%
    {\psfragfig{PhD/Writings/QZS/fig/cons-stableN1-1}}
  ...
  \subbottom[{A range of disturbance displacements $\dmax$
  and a normalised displacement tolerance $\dmin=\num{0.1}$.
  \figlabel{cons-stable-2}}]{%
  {\psfragfig{PhD/Writings/QZS/fig/cons-stableN1-2}}
...
\begin{figure}[b!]
  \parbox[c]{0.5\textwidth}{\psfragfig{PhD/Writings/QZS/fig/nl}}
  \hfill
...
the frequency response is not affected \secref*[vref]{skyhook-intro}.
...
\Figref{variance-amp} shows the variance gain (\secref*[vref]{variance-gain}) for a magnetic system with
...
\Tabref{variance-amp} shows some data from the simulated responses, including the maximum displacement of the spring towards the \qzs/ position, and the nonlinearity measures $\nk$ for each amplitude of excitation.
...
as $\ndispZ_{\text{max}}$ tends towards zero, the motion of the magnetic spring moves closer to the \qzs/ position. Note that the pronounced nonlinearity seen in \figref{variance-amp} belongs to a system that is approaching its bounds of stability, reaching \SI{0.24}{mm} below the \qzs/ position in its most extreme displacement.

\begin{figure}
\let\labelsize\footnotesize
\psfragfig{\jobname/variance-gaps}
\caption
...
resonance peak. This effect could be mitigated by the application of absolute velocity feedback control (`skyhook damping') to the system, which will be discussed in \chapref{xpmt}.

...
In this chapter, a summary of various electromagnetic theories used in the remainder of this thesis is presented.
The first section covers the basic terminology and physics behind magnetic fields, followed (\secref*{magprop}) by a discussion of the properties of permanent magnets.
\Secref{magnet-forces} introduces the methods for calculating the force and torque between permanent magnets and coils, followed by an extensive review (\secref*{cuboid}) on analytical equations for forces and torques between cuboid magnets.
The chapter concludes on the forces between cylindrical magnets (\secref*{cyl-forces}), in which a new expression for the force between coaxial cylindrical magnets is presented.
...
Even in the presence of an external magnetic field, the magnetic flux density $\magB$ inside a permanent magnet is the vector sum of $\magH$ and $\magM$, whereas outside the magnet,  the magnetisation $\magM=0$ and the magnetic flux density $\magB$ is related to the magnetic field strength $\magH$ by a constant.
...
Despite having strong remanence and coercivity values, rare earth magnets can be modelled as having a relative permeability of around $\permrel=\num{1.05}$ under most circumstances (see later discussion on the second quadrant of \figref{BHcurve-strong}), and for most published analytical expressions for field strength the difference from unity is neglected \parencite{kremers2013-ietm}.
...
If the external magnetic field is sufficiently large, the material becomes completely magnetised (such that most of its dipoles are in alignment) and this magnetisation will be retained even once the applied magnetic field is removed.
...
\begin{figure}
  \centerline{
    \subbottom[Strong magnet with `linear' second-quadrant.\figlabel{BHcurve-strong}]{\asyinclude{\jobname/bhcurve}}\qquad
...
The behaviour is shown by the \bhcurve/ of the magnetic material, an example of which is shown for an ideal strong and weak magnet in \figref{BHcurve}.
...
In an ideal magnet with a \bhcurve/ that is linear in the second quadrant, the maximum energy product is directly related to its saturation magnetisation via \cite{campbell1994}
\begin{dmath}[label=bhmax]
  -\BHmax = \permvac\gp{\frac{\Msat}{2}}^2
\end{dmath}.
It can be seen in \figref{BHcurve}(a) that for strong permanent magnets the region of the curve in the second quadrant does indeed approximate a linear function.
...
When using finite element analysis to model rare earth magnets that can be assumed to have a \bhcurve/ that is linear in the second quadrant, it can be necessary to instead define the permeability and coercivity of the magnets.
These three terms are related in this case using the relation
\begin{dmath}
\coerce = \frac{\remanence}{\perm\permvac}
\end{dmath}.

The final term of interest in the \bhcurve/ is known as `intrinsic coercivity' $\coerce{}_i$, which is the magnetic field strength required to completely magnetise a magnetic domain (\ie, a collection of dipoles) or to completely reverse the polarity of magnetisation for the same.
...
\item Flux lines enter (non-saturated) ferrous material at right angles in a low-permeability surrounding.
...
the saturated material. Saturation is the limiting factor for the design of magnetic shielding.
\item Considering a unit magnetic dipole, flux lines travel from north to south magnetic poles in closed loops.
\item Permanent magnets are made up of a very large number of unit magnetic dipoles.
\end{enumerate}
...
When deriving the equations for the forces between magnets in \secref{magnet-forces}, it was assumed that each magnet had a constant and uniform magnetisation and a relative permeability of unity.
...
For example, the magnetic field of a neodymium grade \maggrade{35} cylindrical magnet with diameter \SI{100}{mm} and thickness \SI{30}{mm} was compared against theoretical calculations (performed with \ANSYS/) \cite{bruno2011-honoursthesis}.
While the \bhcurve/ of the magnet is unknown, for rare earth magnets it can be assumed that they have a linear relationship in the second quadrant of the \bhcurve/.

To perform these measurements, a gauss meter (Hirst Magnetic Instruments GM07 Gaussmeter) was used to manually sample the magnetic flux density at a number of locations relative to the centre of the top face of the magnet.
The results are shown in \figref{gauss-meas}.
While the comparison across axial position (\figref*{gauss-vertical}) only shows that the measured magnetic field is lower than expected in the near field, the lateral position results (\figref*{gauss-radial}) indicates why: the magnetic field is lower than expected closer to the centre of the magnet.
While some of this discrepancy will be caused by the assumption of unity relative permeability \parencite{kremers2013-ietm}, this cannot account for the large difference between theory and measurement seen in \figref{gauss-meas}.
One possibility may be that the magnet is not uniformly magnetised due to its large radius.
As a result of this discrepancy in field strength, smaller forces than predicted were measured when using this magnet in repulsion with another large magnet.
...
\section{General techniques for calculating forces between magnets}
...
\Eqref{charge-force,current-force} are general recipes for deriving equations to calculate the forces between permanent magnets of arbitrary geometry, and a similar formulation allows for the modelling of electromagnetic coils as well.
In certain circumstances, the geometry of the permanent magnet can dictate which integral expression should be used; \eg, triangular magnets lend themselves towards the charge model \parencite{janssen2010-compel}.
In other cases is not always clear before attempting a solution to the integrals which of \eqref{charge-force,current-force} will be more suitable in a given situation.
...
This discretisation method is avoided in this thesis due to the limited advantage it has over using the semi-analytical approach that uses numerical integration directly.
...
\end{dmath}.
For $N$ nested integrals, in which $f$ is integrated over variables $x_i$ from $u_i\fn{0}$ to $u_i\fn{1}$:
\begin{dmath}
...
\phi_z\fn{\cornerd} &= -\half\gp{\cnrx^2-\cnrz^2}\Log{\cnrl+\cnry}+\cnrx\cnry\Log{\cnrl-\cnrx}+\cnrx\cnrz\arctan\fn{\tfrac{\cnrx\cnry}{\cnrl\cnrz}}+\half \cnrl\cnry \,.
\end{split}
\end{align}
...
\kappa_y &= -\half\cnry+\frac{\cnrx^2\cnry}{\cnrx^2+\cnry^2} - \frac{\cnrx\cnry\cnrz}{\cnry^2+\cnrz^2}-\cnrx\ArcTan{\frac{\cnrx\cnrz}{\cnry\cnrl}} + \cnry\Log{\cnrz+\cnrl}  \,,\\
...
These equations, particularly \eqref{nforce}, are useful in \secref{qzs} for the analysis of a magnetic system in which the magnet size can be factored away from consideration.
...
\begin{figure}
  ...
  \hspace{-0.8cm}%
  \begin{subfigure}
    \psfragfig{\jobname/charp-rotation-forces}
    \caption{\SI{20}{mm} vertical displacement.}
  \end{subfigure}
  \hfil
  \begin{subfigure}
    \psfragfig{\jobname/charp-horiz-rotation-forces}
  ...
  \lofcaption{Vertical and horizontal forces on a rotating magnet
    due to a fixed one \vs\ rotation angle for fixed horizontal and vertical displacements.}{ The two magnets are \SI{1}{T} \SI{10}{mm} cubes.}
...
\lofcaption{
  ...
  Upper magnet rotates on a lever arm around point $O_m$ of length $z_l=\SI{53}{mm}$ to the magnet centre.
}
\figlabel{rot-geom}
\end{figure}
...
Of the tests performed, two sets of results are shown in \figref{magrotmeas}:
\begin{itemize}
\item \maggrade{45} \SI{15x20x10}{mm} cuboids, and
\item \maggrade{52} \SI{25x25x10}{mm} cuboids.
\end{itemize}
...
\begin{figure}
  \begin{wide}
  \hspace{-1cm}%
  \subbottom[\SI{15x20x10}{mm} \maggrade{45} cuboid magnets.]{\psfragfig{PhD/magrotate/Results/rot-cuboid-15x10x20}}\hfil
...
\end{gather}
where $\cdcnrF_{j,l,q}$ is the vector of magnet side lengths as defined in \eqref{rRvec}.
...
\begin{figure}[tb!]
\subbottom
  [Two geometrically-symmetrical node interactions.
\figlabel{mag-nodes}]
  {\hspace*{-1cm}\asyinclude{\jobname/mag-nodes}}
\hfill
\subbottom
  [Two node interactions for a smaller magnet and a larger magnet.
\figlabel{nodes-bigsmall}]
  {\quad\asyinclude{\jobname/nodes-bigsmall}\quad}
\caption{Two examples of interacting magnetic nodes.}
\end{figure}

\enlargethispage{\baselineskip}

Second, consider a fixed magnet of size $\inlinevect{2a,2b,2c}$ reacting with another magnet of some fixed vertical distance away. Compare two cases for the second magnet: the first of equal size as the first with $\inlinevect{2A, 2B, 2C}=\inlinevect{2a, 2b, 2c}$, and the second of greater size with $\inlinevect{2A, 2B, 2C}=\inlinevect{6a, 6b, 2c}$.
...
As part of the work of this thesis, a simplification of the force equation of \citeauthor{ravaud2010-pier} has been developed \cite{robertson2011-ietm}.
...
\lofcaption{Two-dimensional side view of a system composed of two coaxial cylindrical magnets with a generated force on the second magnet.}
{

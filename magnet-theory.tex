...
A good reference book which inspired this section is\textcite{campbell1994}.
...
called the magnetic flux density, which a measure of the
total flux per area; and $\magH$ is the magnetic field strength,
...
Finally, the permeability $\perm$ of a material (generally
...
fields. The relative permeability $\permrel$ is the ratio of
...
remanence of the magnet, $B_r$. This value is equal to
$\permVac\Msat$ and occurs when there is no external
magnetic field. The other is the coercive force, $H_c$, which
...
demagnetisation curve. This is because the energy used for
...
potential, this is known as the maximum energy product
...
There are two methods that will be outlined here for calculating the magnetic field of a permanent magnet, known as the charge and current models. Respectively, these consist of modelling the magnets as having two surfaces of magnetic charge, or modelling the magnet as being composed of an equivalent surface current density.
...
The magnetic field from `pyramidal frustrum' magnets \cite{compter2010-ietm} is calculated by decomposing the pyramidal frustums into triangular and quadrilateral current sheets of non-square orientation.
...
Negative magnetisation corresponds to a secondary magnet `facing down' and results in forces of reversed sign.

The original expression for the forces between magnets is shown here as terms within will be referred to later.
It is compactly written as six nested summations of intermediate expressions in $x$, $y$, and $z$ directions:
...
The first term of the $z$-component of \eqref{phi-zz} is $-uv\log(r-u)$.
All other terms will be the same since $v$, $w$, and $r$ are constant between the two cases in this example.
...
The verification curve of the results of \textcite{akoun1984}, calculated using \eqref{total-force} with the `magnetforces' function, is shown in \figref{akoun-verify} for the following system:
\begin{align}
\begin{split}
...
The verification curve of the results shown by \textcite{janssen2009-sensorletters} is shown in \figref{janssen} for the following system:
\begin{align}
\begin{split}
...
These simplified equations are printed here to emphasise the $\mdim^2$ relationship shown in \eqref{magforce} and the $\mdim$ relationship in \eqref{magstiffness}.
...
The geometry of the two-magnet system is shown in \figref{akoun}, in which the magnets have side lengths $\bm s = [2a, 2b, 2c]\T$ and $\bm S = [2A, 2B, 2C]\T$ respectively and the distance between their centres is given by $\bm d=[\alpha,\beta,\gamma]\T$. The calculations always assume that the first magnet is fixed and force is acting on the second magnet. The signs must be reversed to obtain the forces acting on the first magnet.
...
\end{dmath}.
\figref{coil-param} shows the parameters used in the simulations and
\figref{ex1} displays the respective outputs calculated with the old equation using Mathematica and the new equation using Matlab.

In the simulation results, the magnet experiences zero force when it is axially centred inside the coil. With axial displacement, a restoring force is applied by the coil in the opposite direction of displacement. As shown in \figref{ex1}, the results are identical for both the original and the simplified equations.
...
\hspace{-4cm}\asyinclude{PhD/Figures/Coil/coil-coil.asy}\hspace{-2cm}%
  ...
  \bottomrule
  \end{tabular}
\lofcaption{Parameters used for the example calculations with results shown in \figref{ex1}.}{ The schematic indicates the direction of current flow and the north and south faces of the magnet. A position of positive displacement $z$ is shown.}
\figlabel{coil-param}
\end{figure}

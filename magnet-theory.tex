...
\Eqref{charge-force,current-force} cannot be solved analytically for complex geometries as the integrals become intractable; numerical integration must be used in this case \cite{charpentier2001}.
...
The $\Fzynode$ terms required for calculating the `force between nodes' for orthogonal magnets can now be written as:
\begin{align}
\begin{split}
...
The geometry of the two-magnet system is shown in \figref{akoun}, in which the magnets have side lengths $\mwX = [2\hwxX, 2\hwyX, 2\hwzX]\T$ and $\mwF = [2\hwxF, 2\hwyF, 2\hwzF]\T$ respectively and the distance between their centres is given by $\magcd=[\alpha,\beta,\gamma]\T$. The calculations always assume that the first magnet is fixed and force is acting on the second magnet. The signs must be reversed to obtain the forces acting on the first magnet.

As shown earlier in \eqref{akoun},
\textcite{akoun1984} provide the force expressions for magnets with vertical magnetisations.
This force is denoted below as $\Fzz\fn{\mwX, \mwF, \magcd, \magn1, \magn2}$ as a function of the magnet sizes, the distance between them, and their magnetisation magnitudes $\magn1$ and $\magn2$.
...
This force is denoted below as $\Fzy\fn{\mwX, \mwF, \magcd, \magn1, \magn2}$.

The force between a vertically-magnetised magnet and one with magnetisation in the horizontal $\ax$ direction can be calculated by applied a rotational transformation to $\Fzy$ around the $\az$ axis.
That is,
...
that all contain the force expression of interest.
...
\lofcaption{
  ...
  Upper magnet rotates on a lever arm of length $l_m=\SI{60}{mm}$ around point $O_m$.
}
\figlabel{rot-geom}
\end{figure}
...
Of the tests performed, two sets of results are shown in \figref{magrotmeas}: N45 \SI{20x20x20}{mm} cubes and N52 \SI{25x25x10}{mm} square-face cuboids.
...
\begin{figure}
  \begin{wide}
  \subbottom[\SI{20x20x20}{mm} N45 cube magnets.]{\psfragfig{PhD/magrotate/Results/rot-cube-20}}\hfil
  \subbottom[\SI{25x25x10}{mm} N52 cuboid magnets.]{\psfragfig{PhD/magrotate/Results/rot-cuboid-25x10x25}}
  \end{wide}
  \caption{
...
The forces between non-coaxial cylindrical magnets were considered by \textcite{agashe2008-applphys,vokoun2009-jmmm}, with a very recent publication in press with the most efficient solution yet presented \cite{conway2013-ietm}.

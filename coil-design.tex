...
The analysis of a single \dof/ actuator shows that it can be possible to optimise coil size, magnet size, and wire diameter to achieve a given force maximum.
...
A new equation is devloped for calculating the force between cylindrical coils and magnets with eccentric displacement, and some examples of alternate electromagnet designs are given based on the modelling results shown.
...
The $\turnsCoil\currentCoil/\gp{\oradiusCoil-\iradiusCoil}$ term in the force equation between a magnet and coil is the cause of some subtle considerations when designing coil--magnet actuators.
...
The previous section has outlined a optimisation technique to achieve the maximum peak force given a fixed magnet volume and input resistance.
...
\begin{figure}
  ...
  \end{subfigure}
  \end{wide}
  \caption{Dual coil system.\fxfatal{xoxo}}
  \figlabel{dual-coil}
\end{figure}

For two coaxial coils with a distance $G$ between their centres, superposition can be used to calculate the forces $F_1$ and $F_2$ generated between each and a permanent magnet.
The axial distance between the permanent magnet and the two coils is denoted by
$d_1$ and $d_2$.
The total force on the magnet is given by
\begin{dmath}
F_{\mathrm{T}}\fn{d_1,G}
  =F_1(d_1)+F_2(d_2)
  =F_1(d_1)+F_2(-G+d_1)
  =F_1(d_1)+F_1(G-d_1)
\end{dmath}.

...
\figref{fig:filament-shell} shows such a filament model for the interaction between a thin coil (representing a permanent magnet) and a thick coil.
...
Under eccentric displacement in the radial direction, $\eccen$, the limits of integration must be expressed in a translated coordinate system (see \figref{fig:int-offset}) and the original integral must be expressed with additional terms to take this into account.
...
\lofcaption{Maximum normalised force per ampere of current, optimised by magnet and coil geometry as a function of wire diameter.}{ Results are shown for three values of coil impedance while the magnet volume is fixed at $\volMag=(\SI{20}{mm})^3$.}
...
\caption
[Maximum peak force $\fmax$ versus wire diameter.]
{Maximum peak force $\fmax$ versus wire diameter calculated by multiplying the normalised maximum force by the maximum current rating for that wire diameter (\figref{magcoil-maxcurr,magcoil-ratiomax-norm}, \resp).}

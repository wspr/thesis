...
\subsubsection{Unstable vertical spring}

Unstable vertical ($k_z<0,k_x=k_y=-\half k_z>0$); discussed in the literature review in the context of active control methods (\secref{levitation-control}).
...
\subsubsection{Stable vertical}

($k_z>0,k_x=k_y=-\half k_z<0$); vertical forces analysed in detail in \secref{unstable-vertical}.
...
\subsubsection{Quasi--zero stiffness}
\seclabel{qzs-basics}

($k_x=k_y=k_z=0$); analysed in detail in \secref{qzs}
...
First introduced by \textcite{nijsse2001}, the zero stiffness spring, \figref{zero-stiffness-spring}, uses equal and opposite magnet pairs to create a force--displacement relationship that has a region of zero slope, shown in \figref{zspring-vary-gap} for half-inch magnet cubes.
When vertically centred, the spring achieves \qzs/ in all three translatory directions.
A detailed analysis of this magnet arrangement for \qzs/ is undertaken later in \secref{qzs}.
...
($k_x<0,k_y=k_z=-\half k_x>0$); \secref{hspring}.
...
\subsubsection{Double stable}


\Figref{hvspring} shows how the two springs discussed so far, horizontal and vertical, may be combined.
...
\subsubsection{Vertical zero stiffness}
($k_z=0,k_y=-k_x>0$).
...
It is also possible to combine the zero stiffness spring with the horizontal spring to add stability to one horizontal degree of freedom.
Such a design is shown in \figref{zhv-spring} and its forces analysed in three degrees of freedom in \figref{zhvspring-move,zhvspring-move-z}.
There is a single magnet dimension gap between the floating magnet and all fixed magnets; again, half-inch cubes are used to model the system.
...
Translating the spring in two directions simultaneously gives a better
picture of the stability of the spring. For example, motion in the
\x-direction showns zero force in the \y, and vice versa.
The addition of the horizontal magnets produces an asymmetry in the total vertical force characteristic of the system, but it still retains an inflection point at which \qzs/ can be realised.


\subsubsection{Isoforces}
\seclabel{isoforces}


Analysing the force--displacement characteristics for these devices remains quite challenging.
To indicate the difficulty of visualising the complex force profile these devices exhibit, representative force component contours are plotted in \figrangeref{vattr-forces}{zhv-forces} against displacement in each direction; these contours are termed `isoforces' as they trace contours of equal force.
...
\isoforces{vrepl}{Isosurface forces of the repelling vertical spring.}
\isoforces{z}{Isosurface forces of the zero stiffness spring.}
\isoforces{h}{Isosurface forces of the horizontal spring.}
\isoforces{hv}{Isosurface forces of the combination spring.}
\isoforces{zhv}{Isosurface forces of the combination zero stiffness spring.}
...
\subfloat[Without rotation.]
  {\pregen{\includegraphics[scale=0.8]{magcode/examples/oblique/fig/mbq-angle-schem-1}}}\qquad
\subfloat[With rotation shown in black; the unrotated position, as in (a), is shown in light grey.]
  {\pregen{\includegraphics[scale=0.8]{magcode/examples/oblique/fig/mbq-angle-schem-2}}}\qquad
\subfloat[With small angle approximation of zero magnet rotation shown in colour; the rotated magnets, as in (b), are shown in black.]
  {\pregen{\includegraphics[scale=0.8]{magcode/examples/oblique/fig/mbq-angle-schem-3}}}
\end{wide}

...
(100 for \price[\usdollar]{6.00}) to one inch cubes, (\price[\usdollar]{14.85}
each), and even larger. \note{Prices correct as of April 2009.} Price has an
...
Permanent magnets in two different shapes are examined here: cuboid and cylindrical.

\subsection{Cuboid magnets}

Consider the basic magnetic spring shown in \Figref{schematic}, consisting of two magnets separated by a displacement, $x$ (measured between the near faces), and generating a repulsive force, $F$, between them. The magnets have square facing sides, a height-to-width ratio of $\gamma=a/b$, and a fixed volume $V$; the height of each magnet is $a=\sqrt[3]{V\gamma^2}$ and the face size width (and length into the page) is $b=\sqrt[3]{V/\gamma}$.

The magnetic forces between the magnets can be calculated by applying the theory of \textcite{akoun1984}, where the force $F=F_m(V,\gamma,x)$ is a function of magnet volume $V$, size ratio $\gamma$, and displacement $x$.

Such forces were calculated between these magnets for a magnet volume $V=(\SI{10}{mm})^3$ over a displacement $x$ from \SIrange{0}{10}{mm} and a magnet size ratio $\gamma$ from $0.1$ to $1$. Note that the forces were calculated with a magnetisation of \SI{1}{T} for both magnets, essentially normalising the output forces by the magnetisation strength. The MATLAB code used to calculate these results is located in the script {examples/mag\_ratio.m} in the code repository \url{http://github.com/wspr/magcode}.

In order to compare the force versus displacement characteristics for a range of magnet size ratios, we will normalise the forces by the force $F_s=F_m(V,1,x)$; that is, the force for a magnet size ratio $\gamma=1$. \Figref{ratio1,ratio2} show the normalised force $\bar F=F/F_s$ as a function of displacement $x$ over a range of magnet size ratios $\gamma$. The figures are drawn as separate graphs in order to avoid overlap of the curves; size ratio $\gamma$ varies from $0$ to $0.4$ in \Figref{ratio1} and from $0.4$ to $0.8$ in \Figref{ratio2}. It can be seen from the two graphs that a magnet size ratio $\gamma$ of around $0.4$ produces the greatest forces; for values both less and greater than $0.4$, the normalised force curves decrease.

\begin{figure}[p]
\centering
\psfragfig{examples/fig/magratio1}
\caption{Normalised force $\bar F=F/F_s$ as a function of displacement $x$ for a set of magnet ratios $\gamma=\{0.1,0.2,0.3,0.4\}$.}
\figlabel{ratio1}
\end{figure}

Some small overlap in the force curves for $\gamma=0.4$ and $\gamma=0.5$ is seen in \Figref{ratio2}. This indicates that the optimum magnet size ratio (to maximise the force) is dependent on the displacement between the magnets. \Figref{magratio} shows the magnet force varying as a function of magnet size ratio $\gamma$ with a set of curves corresponding to fixed displacements from \SIrange{1}{10}{mm}. For each curve, there is a local maxima in the force; this corresponds to the magnet size ratio that produces the greatest force at that displacement. While the magnet size ratio that produces the greatest forces varies with displacement, the graph shows that the optimum magnet size ratio remains around $\gamma\approx0.4$.

\begin{figure}[p]
\centering
\psfragfig{examples/fig/magratio2}
\caption{Normalised force $\bar F=F/F_s$ as a function of displacement $x$ for a set of magnet ratios $\gamma=\{0.4,0.5,0.6,0.7,0.8\}$.}
\figlabel{ratio2}
\end{figure}

\begin{figure}[p]
\centering
\psfragfig{examples/fig/magratios}
\caption{Force between two magnets as a function of magnet height--width ratio $\gamma=a/b$ at a number of fixed displacements from $\SIrange{1}{10}{mm}$. The positions of maximum force are shown with dots, indicating that magnet size ratio to achieve the maximum force is in the region around $\gamma=0.4$.}
\figlabel{magratio}
\end{figure}
...
\note{Remanence $B_r=\SI{1.2}{T}$ and coercivity $H_c\approx\SI{900}{kA\cdot m^{-1}}$}

...
Single-axis magnetic springs are discussed in \secref{unstable-vertical}, comparing cuboid and cylindrical magnets.
A variety of magnetic spring designs are considered in \secref{magcuboid}, especially in regard to the stability of different designs; it is shown that it is possible in theory to design a rotationally-stable magnetic spring.

The chapter concludes with a theoretical (\secref*{oblique}) and experimental (\secref*{mbq-prototype}) analysis of a new magnetic spring design using inclined permanent magnets that decouples load bearing and resonance frequency.
...
If the constraint is removed, it will be naturally unstable horizontally due to Earnshaw's theorem (\secref*{earnshaw}).
...
In order to compare the force versus displacement characteristics for a range of magnet size ratios, the forces were normalised by the force for cube magnets $\magforce=\Fzz\fn{\volMag,1,\displ}$. \Figref{magratios} shows the normalised force $\forceMag/\forceMag_s$ as a function of displacement $\displ$ over a range of magnet size ratios $\ratioCub$. The figures are drawn as separate graphs in order to avoid overlap of the curves; size ratio $\ratioCub$ varies from \num{0} to \num{0.4} in \Figref{magratio1} and from \num{0.4} to \num{0.8} in \Figref{magratio2}. It can be seen from the two graphs that a magnet size ratio $\ratioCub$ of around \num{0.4} produces the greatest forces; for values both smaller and greater than \num{0.4}, the normalised force curves decrease.
...
\subfloat
  [Magnet ratios $\ratioCub=\{0.1,0.2,0.3,0.4\}$
   \figlabel{magratio1}]
  {\psfragfig{magcode/examples/fig/magratio11}}
\subfloat
  [Magnet ratios $\ratioCub=\{0.4,0.5,0.6,0.7,0.8\}$
   \figlabel{magratio2}]
  {\psfragfig{magcode/examples/fig/magratio22}}
\end{wide}
...
To investigate this, the force versus displacement characteristic between two cube magnets was calculated for displacement perpendicular and orthogonal to the magnetisation directions, as shown in \figref{xz-magforce-schem}.
...
Similar results were shown in the discussion of cuboid magnet forces due to rotations (\secref*{french-equations}).

\begin{figure}
\begin{wide}
  \subfloat[Schematic of the two magnet orientations.
    \figlabel{xz-magforce-schem}]{\quad\asyinclude{\jobname/twomagdir}\quad}
  \qquad
  \subfloat[Forces in two directions versus displacement.
    \figlabel{xz-magforce-plot}]{\psfragfig{magcode/examples/fig/xz-magforce}}
\end{wide}
...
The force is calculated using \eqref{simpl4} for a magnet volume of $\volMag=\gp{\SI{10}{mm}}^3$ with force \vs\ magnet ratio over a range of displacements shown in \figref{magratio-cyl}.
...
\end{align}
In other words, for equal $\ratioLengthArea$ a cylindrical and cuboid magnet will have the same face area as shown in \figref{squarecircle}.
  \note{Relating to a geometrical problem of historical interest known as `squaring the circle' \cite{hobson1913-squircle}.}
Calculating the force versus displacement characteristics across magnet ratio $\ratioLengthArea$ between cylindrical and cuboid magnets ($F_{\mathrm{cyl}}$ and $F_{\mathrm{cub}}$, \resp) shows that the cylindrical magnets produce greater forces across almost the entire ranges of displacement and magnet ratio (\figref*{cub-cyl-ratios-plot}).
...
A vertical, planar, two-magnet system was defined, shown in \figref{coupling-schem}, in which the lower magnet was horizontally oscillated and the resulting motion of the upper suspended magnet was investigated.
...
A numerical time-domain differential equation solver (Mathematica 7.0's \texttt{NDSolve}) was used to simulate the dynamics of this system from initial relative position $(0,0)$ at which the force due to gravity was balanced by the magnetic force. The displacement trace of the numerical solution is shown in \figref{coupling-trace}, where the system can be seen to initially follow a non-regular path until reaching a steady state figure-eight limit cycle.

\begin{figure}
  \subbottom[Schematic.\figlabel{coupling-schem}]{\asyinclude{\jobname/coupling}}\hfill
  \subbottom[Relative displacement trace.\figlabel{coupling-trace}]{\psfragfig{\jobname/coupling-oscillations}}
  \lofcaption{Oscillations over time of the displacement of a suspended magnet that is excited through horizontal vibrations of the base magnet.}{ Starting from $(0,0)$, the resulting figure-eight trajectory shows that the horizontal vibration couples to the vertical forces.}
...
\begin{figure}
\begin{wide}
  \subbottom[Equilibrium positions of a suspended magnet bearing a range of
  masses varying with horizontal displacement.\figlabel{mag-coupling-equil}]
  {\psfragfig{\jobname/mag-coupling-equil}}
  \hfil
  \subbottom[Ratio of vertical to horizontal displacement (`coupling ratio') of
  a suspended magnet bearing a range of masses varying with horizontal
  displacement.\figlabel{mag-couplingratio-equil}]
  {\psfragfig{\jobname/mag-couplingratio-equil}}
\end{wide}
  \caption{
...
In the following sections (\secref*{unstable-spring}--\secref*{spring-zhv}), six different magnetic spring configurations are introduced based around cuboid magnets.
These are compared with each other in \secref{isoforces} with \figrangeref{vattr-forces}{zhv-forces} showing for each design their contours of force \vs\ displacement in each direction.
...
\begin{figure}[t]
  ...
  \lofcaption{\QZS/ vertical spring forces \vs\ vertical displacement from the centred position.}{ The magnets are \SI{20}{mm} cubes with \SI{1}{T} magnetisation, and there is \SI{80}{mm} between the top and bottom magnet centres. Dashed region for the summation of forces corresponds to negative (unstable) stiffness.}
...
First introduced by \textcite{nijsse2001}, the \qzs/ magnetic spring uses equal and opposite magnet pairs to create a force--displacement relationship that has a region of zero slope, shown in \figref{zspring-vary-gap} for \SI{1}{T} \SI{20}{mm} magnet cubes.
...
In this case, the operating position of the floating magnet is situated closer to the lower magnet, which is as depicted in \figref{spring-zk3}.
...
Unlike the vertical magnetic springs discussed previously, a horizontal arrangement of magnets in attraction, \figref{hspring}, provides stability in the vertical direction with only a single degree of horizontal instability.
...
\end{dmath}.
A demonstration of the force--displacement curves for this spring is shown in \figref{zhvspring-move-z}, for a system with \SI{20}{mm} cube magnets with \SI{1}{T} magnetisation and a nominal gap of \SI{30}{mm} between the centres of the floating magnet and the horizontal magnets and \SI{40}{mm} nominal gap for the vertical magnet centres.
...
\subsection{Isoforces and isostiffnesses for magnetic springs}
\seclabel{isoforces}
...
As the \threeD/ forces vary with displacement in three \dofs/, it is not possible to directly visualise the behaviour of these systems under arbitrary displacement.
An attempt to do so here highlights the difficulties involved.
In this case, representative force component contours are plotted against displacement in each direction separately; these contours are termed `isoforces' as they trace contours of equal force (\figrangeref{vattr-forces}{zhv-forces}) and `isostiffnesses' with contours of stiffness (\figrangeref{vattr-stiffnesses}{zhv-stiffnesses}).


\newcommand\isoforces[4][10mm]{%
  \begin{figure}[p]
    \begin{wide}
      \vspace*{-0.5cm}
      \centerline{
      \hspace*{#1}%
      \subfloat[$\ax$ forces.\figlabel{iso-#2-x}]{%
        \psfragfig
          {PhD/Simulations/Single_magnets/all_springs/frag2013/#2-xforce}%
      }\hspace*{-3mm}%
      \subfloat[$\ay$ forces.\figlabel{iso-#2-y}]{%
        \psfragfig
          {PhD/Simulations/Single_magnets/all_springs/frag2013/#2-yforce}%
      }\hspace*{-3mm}%
      \subfloat[$\az$ forces.\figlabel{iso-#2-z}]{%
        \psfragfig
          {PhD/Simulations/Single_magnets/all_springs/frag2013/#2-zforce}%
      }%
      }%
    \end{wide}
    \caption[#3.]{#3 (\secref*{#4}).}
    \figlabel{#2-forces}
  \end{figure}
}

\isoforces[-13mm]{vattr}{Isoforces of the unstable vertical spring}{unstable-spring}
\isoforces[-13mm]{vrepl}{Isoforces of the stable vertical spring}{spring-repl}
\isoforces[-13mm]{z}{Isoforces of the \qzs/ spring}{qzs-basics}
\isoforces[13mm]{h}{Isoforces of the horizontal spring}{hspring}
\isoforces[13mm]{hv}{Isoforces of the combination spring}{spring-hv}
\isoforces[13mm]{zhv}{Isoforces of the combination \qzs/ spring}{spring-zhv}



\renewcommand\isoforces[4][10mm]{%
  \begin{figure}[p]
    \begin{wide}
      \vspace*{-0.5cm}
      \centerline{
      \hspace*{#1}%
      \subfloat[$\ax$ stiffnesses.\figlabel{isos-#2-x}]{%
        \psfragfig
          {PhD/Simulations/Single_magnets/all_springs/frag2013/#2-xstiffness}%
      }\hspace*{-3mm}%
      \subfloat[$\ay$ stiffnesses.\figlabel{isos-#2-y}]{%
        \psfragfig
          {PhD/Simulations/Single_magnets/all_springs/frag2013/#2-ystiffness}%
      }\hspace*{-3mm}%
      \subfloat[$\az$ stiffnesses.\figlabel{isos-#2-z}]{%
        \psfragfig
          {PhD/Simulations/Single_magnets/all_springs/frag2013/#2-zstiffness}%
      }%
      }%
    \end{wide}
    \caption[#3.]{#3 (\secref*{#4}).}
    \figlabel{#2-stiffnesses}
  \end{figure}
}

\isoforces[-13mm]{vattr}{Isostiffnesses of the unstable vertical spring}{unstable-spring}
\isoforces[-13mm]{vrepl}{Isostiffnesses of the stable vertical spring}{spring-repl}
\isoforces[-13mm]{z}{Isostiffnesses of the \qzs/ spring}{qzs-basics}
\isoforces[13mm]{h}{Isostiffnesses of the horizontal spring}{hspring}
\isoforces[13mm]{hv}{Isostiffnesses of the combination spring}{spring-hv}
\isoforces[13mm]{zhv}{Isostiffnesses of the combination \qzs/ spring}{spring-zhv}
...
The colour scale goes through the spectrum from orange to purple as the normalised forces/stiffnesses range from negative to positive.

The vertical components are easiest to comprehend.
\Figref{iso-vattr-z} shows that as the floating magnet is displaced upwards, the magnitude of the vertical force increases (becoming more blue).
...
Despite the magnitude of the forces being vertically upwards, the stiffnesses in \figref{isos-vattr-z,isos-vrepl-z} shows the negative (orange) stability in the former case and positive (blue) stability in the latter.
...
The stiffnesses, positive for \figref{isos-vattr-x,isos-vattr-y} and negative for \figref{isos-vrepl-x,isos-vrepl-y} emphasise this behaviour.
...
Note, however, that due to the differences in magnetisation direction between the magnets, a different stability pattern is seen; increases in $\ay$ and $\az$ displacement respectively both result in restoring forces (note the stiffnesses, \figref*{h-stiffnesses}, which are positive in two axes).

The final isoforce graphs are the two `combination' springs, \figref{hv-forces,zhv-forces}, involving superposition of isoforces seen previously.
...
Only a preliminary analysis of these has been conducted here; in \secref{oblique} a different spring type using inclined magnets is covered in detail, and later in \secref{qzs} the design characteristics of the zero stiffness spring are examined in more detail.
...
\psfragfig{\jobname/choi-stiffness}
\caption{
...
\Figref{choi-xz} shows the effect on the vertical and horizontal stiffnesses due to varying the magnet gap as a function of vertical displacement $\dispvert$; the out-of-plane stiffness is not shown as it is always negative.
...
Finding these regions and plotting the vertical force \vs\ displacement curves against magnet gap (\figref*{choi-force}) demonstrates the useable ranges of the device.
...
\lofcaption{
  ...
  dashed lines indicate regions of negative stiffness in the vertical and/or horizontal directions.
}
\figlabel{choi-force}
\end{figure}

This idea of tuning the magnet characteristics to achieve certain stiffness properties is explored in more detail in \secref{oblique} using inclined magnets.
...
\begin{figure}[b!]
   ...
   \lofcaption{An example of stabilising rotationally-unstable springs.}{ Forces due to magnets and gravity, $\forceMagnet$ and $\forceGrav$, are depicted. The rotationally unstable case is shown on the left; the right schematic shows the stabilised coupling.}
   \figlabel{rot-couple}
\end{figure}
...
In this section, a cuboid magnet design with similar principles is shown to demonstrate that it is possible to eliminate rotational instability for a magnetic spring application (also see \secref{choi-spring}).
...
\begin{figure}[p]
  ...
  \begin{tabular}{cr}
    \toprule
      $\leverArm_1$ &  \SI{100}{mm}  \\
      $\leverArm_2$ &  \SI{300}{mm}  \\
      $\springDepth$ &  \SI{100}{mm}  \\
      $\magnetGap$ &  \SI {10}{mm}  \\
      $\lengthCube_1$ &  \SI {10}{mm}  \\
      $\lengthCube_2$ &  \SI  {5}{mm}  \\
    \midrule
      $\dispvert_\equil$ & \SI{-4}{mm} \\
    \bottomrule
  \end{tabular}
  }
  \end{subfigure}
...
\begin{figure}[t]
  ...
  \lofcaption{
    Demonstration of single \dof/ instability.
  }{
    The gradients of all forces and torques are negative (infering stability)
    except for the force along the $\ay$ direction.
  }
...
\begin{figure}
  ...
  \end{wide}
  \lofcaption{
    Moment of the magnetic spring as it rotates around the $\ay$ and $\az$ axes with a varying lever arm for the stabilising magnets.%
  }{
    (The moment around the $\ax$ axis does not vary with lever arm and remains stable.)
    Plots are labelled in terms of the lever arm ratio $\leverArm_1/\leverArm_2$.
  }
...
Spring parameters (shown in \figref{magrotate-schem}) are chosen to illustrate the concept of having a single degree of instability, with forces and moments shown in \figref{demon-stable-rotation}.
...
\end{wide}
  \lofcaption{
    Varying the size of the stabilising magnets for the conceptual single \dof/ magnet spring.
  }{
    Magnet size ratios $\lengthCube_2/\lengthCube_1$ are labelled on each plot or indicated in increasing directions with an arrow.
...
Again using the illustrative parameters of \figref{magrotate-schem}, but with spring depth $\springDepth$ varying from \SIrange{100}{400}{mm}, the forces and moments are calculated and shown in \figref{stablerot-depth}.
...
In contrast to \qzs/ systems, introduced in \secref{qzs-basics} and analysed in more detail later in \chapref{qzs}, which attempt to reduce the stiffness as much as possible, in this section magnetic forces are used in such a way as to yield a larger region of low stiffness.
...
A schematic of the inclined magnetic spring is shown in \Figref{mbq-schematic}.
...
\begin{figure}
\centering
\asyinclude{\jobname/oblique.asy}
\lofcaption {
  Schematic of the inclined magnet spring.
}{
...
An assumption is made that there are no magnetic interactions between magnets from one side of the spring to magnets on the other side.
...
\eqlabel{mbq-force}
\end{equation}
To calculate $\mbqforce_{1}$ and $\mbqforce_{2}$ a local coordinate system is defined for each magnet pair at the base magnet, aligned in each respective direction of magnetisation.
Then $\mbqforce_1=\mbqrot\fn{\mbqmagangle} \mbqmforce_1$ and $\mbqforce_2=\mbqrot\fn{\mbqcompmagangle}\mbqmforce_2$, where $\mbqcompmagangle=\pi-\mbqmagangle$, $\mbqmforce_{1}$ and $\mbqmforce_{2}$ are the forces between the magnet pairs in the local coordinate systems of the base magnets, and $\mbqrot\fn{\cdot}$ is the planar rotation matrix given in \eqref[vref]{rotZ}.
...
\begin{align}
  ...
  \mbqmdispl_2 &=
    ...
    \begin{bmatrix}\mbqmagh \\ 0 \\ 0\end{bmatrix} ,
\end{align}
where $\mbqmagh$ and $\mbqoffset$ are parameters defined in \Figref{mbq-schematic}, and $\inlinevect{\mbqhdisp,\mbqvdisp,\mbqpdisp}$ are displacements in the horizontal, vertical, and out-of-plane directions, respectively. In \secref{stabl-3dof} this model will be extended with a small angle approximation to calculate forces and torques due to rotation around the $\mbqpaxis$~axis.
...
By plotting natural frequency as a function of load force in \Figref{mbq-wvf-angle}, it is possible to choose a magnet angle based on a certain load to satisfy a desired natural frequency.
...
\Figref{mbq-wvf-angle-1} shows the natural frequency versus load curve for a magnet offset ratio $\mbqoffset/\mbqunit$ of zero.
...
This is possible as the stiffness in the out-of-the-page direction of \Figref{mbq-schematic} is always negative (\figref*{mbq-kvxyz-gaps-p}), and as a consequence of Earnshaw's theorem (\secref[vref]{earnshaw}) the stiffnesses in each direction must sum to zero; that is, $\mbqhstiff\fn\mbqvdisp+\mbqvstiff\fn\mbqvdisp+\mbqpstiff\fn\mbqvdisp=0$.
...
With fixed magnet offset ratio of $\mbqoffset/\mbqunit=\num{0.2}$ and magnet angle of $\mbqmagangle=\SI{40}{\degree}$, the natural frequency \vs\ force characteristic with volumes from $\mbqvolume=\gp{\SI{10}{mm}}^3$ to $\mbqvolume=\gp{\SI{50}{mm}}^3$ is shown in \Figref{mbq-wvf-vol}, which shows that larger magnet sizes permit larger load forces while also retaining a low natural frequency.
...
In \secref{stabl-v}, the translational stiffness of the system in three directions was discussed in terms of a change in the vertical equilibrium position of the spring (\eg, corresponding to a variation in applied load).
...
\begin{figure}[t]
\psfragfig{magcode/examples/oblique/fig/mbq-angle-schem}
\caption
...
\end{wide}
\caption[Representation of the small angle approximation for magnet rotations.]{Visual representation of the small angle approximation in which the magnet structure rotates but the magnets themselves are assumed to remain parallel to their respective partner.}
\figlabel{mbq-angle-schem-approx}
\end{figure}

The model developed for this system in \Secref{geom} is here extended to calculate torques and allow (small) rotations, both around the $\mbqpaxis$ axis only.
...
{Torque versus rotation for three spring configurations shown in \figref{mbq-rot-ex-diag} with parameters
...
The torsional stability due to rotation is affected by the geometric parameters of the system as shown by example in \figref{mbq-rot-ex}, in which varying the magnet offset ratio changes the rotational stiffness from stable ($\mbqoffset/\mbqunit=\{0.25, 0.5\}$) to unstable ($\mbqoffset/\mbqunit=1$).
Scaled vector diagrams to illustrate these are ideas are shown in \figref{mbq-rot-ex-diag}.
...
\subfloat[$\mbqoffset/\mbqunit=0.25$]
  {\pregen{\psfragfig{magcode/examples/oblique/fig/mbq-rot-ex-diag-1}}}\qquad
\subfloat[$\mbqoffset/\mbqunit=0.5$]
  {\pregen{\psfragfig{magcode/examples/oblique/fig/mbq-rot-ex-diag-2}}}\qquad
...
The parameters shown in \Tabref{mbq-dyn-param} were used for the dynamic simulations.
...
Given the system described in \Eqref{mbq-dyn} and a perturbation of $\mbqhpert=\mbqvpert=\SI{1.5}{mm}$, the resultant dynamics are shown in \Figref{mbq-dyn-xy} as displacements from the equilibrium position of the spring.
...
\subfloat[Horizontal and vertical displacement from the equilibrium position.]
  {\pregen{\psfragfig{magcode/examples/oblique/fig/xmbq-dyn-xy}}}\qquad
\subfloat[Displacement map with colour progressing with time from dark to light.]
  {\pregen{\psfragfig{magcode/examples/oblique/fig/xmbq-dyn-xy-map}}}
\end{wide}
\caption
...
The second stability example constrains horizontal displacement while allowing free rotation of the system. A dynamic simulation was performed with a perturbation of $\mbqvpert=\SI{1.5}{mm}$ and $\mbqrotpert=\SI{3}{\degree}$, for which results are shown in \Figref{mbq-dyn-yr}.
...
\subfloat[Relative vertical displacement $\mbqvdisp-\mbqvdisp_0$.]
  {\psfragfig{magcode/examples/oblique/fig/xmbq-dyn-yr-y}}\qquad
\subfloat[Rotation $\mbqrotz$.]
  {\psfragfig{magcode/examples/oblique/fig/xmbq-dyn-yr-r}}
\end{wide}
\caption
...
\subfloat[Horizontal displacement $\mbqhdisp$.]
  {\psfragfig{magcode/examples/oblique/fig/xmbq-dyn-xyr-x}}\quad
\subfloat[Relative vert.\ displacement $\mbqvdisp-\mbqvdisp_0$.]
  {\psfragfig{magcode/examples/oblique/fig/xmbq-dyn-xyr-y}}\quad
\subfloat[Rotation $\mbqrotz$.]
  {\psfragfig{magcode/examples/oblique/fig/xmbq-dyn-xyr-r}}
\end{wide}
\caption
...
To explore the possibilities raised by the theoretical analysis of the inclined magnetic spring (\secref*{oblique}), an honours project co-supervised by the author was undertaken in 2011 to design and build a prototype \cite{frizenschaf2011-honoursthesis,frizenschaf2011-acoustics2011}.
...
\begin{figure}[p]
\centerline{\includegraphics{PhD/Figures/Oblique/oblique-rig}}
\caption
...
The six degree of freedom system was modelled using an extension of the theory developed in \secref{oblique-planar-theory} for the planar three \dof/ system \parencite{frizenschaf2011-honoursthesis}.
The model predicted the behaviour of the prototype well; \tabref{oblique-rig-resonances} shows the measured and modelled resonance frequency for two magnet angles, which differ by around 5\%.
...
\begin{wide}
\subbottom
  [Resonance frequency compared with predicated values and against the response of a linear spring.\figlabel{oblique-resonance}]
  {\psfragfig{PhD/Experiments/Oblique/resfreq}}%
\hfil
\subbottom
  [Damping ratio.\figlabel{oblique-damping}]
  {\psfragfig{PhD/Experiments/Oblique/damping-ratio}}
\end{wide}
...
It is interesting to compare the significant difference in damping ratio for these two magnet angles (\tabref{oblique-rig-resonances}).
The strongest contributor to the damping ratio in this system is from induced eddy currents in each permanent magnet by the magnetic field of the other (\secref*{damping}), as viscous damping in the bearings and due to air resistance can be assumed to be constant.
...
\begin{figure}
\psfragfig{PhD/Experiments/Oblique/TF}
\caption
...
Integrated accelerometer measurements from the platform were used to generate the control signal for `skyhook damping' (\secref*{skyhook-intro}).
...
Behaviour at higher frequencies is unaffected, as is expected with this method of vibration control (compare \figref[vref]{skyhook-ideal}).

\begin{figure}
\psfragfig{PhD/Experiments/Oblique/velcontrol}
...
Indeed, due to the consequences of Earnshaw's theorem it is expected that there will be significant instability in at least one rotational direction unless specific design effort is expended to ensure stability in all rotational degrees of freedom (\secref*{rotation-freedom}).

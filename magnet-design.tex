...
In \secref{magprop}, the selection of rare earth magnets is discussed, including some experimental results that indicate that large magnet sizes should be tested carefully to ensure they are magnetised homogeneously.
...
Permanent magnets are available in a range of materials; for the purposes of this research only neodymium-boron-iron alloys have been considered, which are inexpensive and widely used nowadays.
...
\begin{figure}
  ...
  \lofcaption{Magnet price versus magnet volume for rare earth magnets.}{
    Data obtained from \KJMagnetics/ for cube magnets of magnetisation grade \maggrade{42}.
    Late 2012 prices are slightly lower than those shown for 2011.}
  \figlabel{mag-price}
\end{figure}

Permanent magnets are graded according to their maximum energy product (\eqref[vref]{bhmax}); \eg, a magnet with grade \maggrade{42} has $\BHmax=\SI{42}{MG.Oe}$.
Since the conversion to metric for the units of $\BHmax$ is $\SI{1}{MG.Oe}=100/\gp{4\pi}$\,\si{kJ/m^3}, the metric remanence $\remanence$ (units Tesla) of a grade $\maggradesym$ magnet can be calculated with
\begin{dmath}[compact]
  \remanence = 2\sqrt{\permvac\BHmax} = 2\sqrt{\maggradesym/100}
\end{dmath},
which corresponds to $\remanence=2\sqrt{0.42}=\SI{1.3}{T}$ for an \maggrade{42} grade magnet.
The analyses performed in this thesis are generally normalised by magnet strength; \ie, the remanence magnetisations for theoretical calculations are usually assumed to be $\remanence=\SI{1}{T}$.

Magnets of grade \maggrade{52} have an approximately 12\% increase in magnetic flux density over \maggrade{42} grade, which corresponds to a force increase of 25\%, for an average cost increase of around 40\% (prices from \KJMagnetics/).
As an aside, the highest-known theoretically achievable remanence magnetisation for a permanent magnet is around \SI{2.4}{T} \cite{sellmyer2002}, corresponding to a force increase of 236\% over \maggrade{42} grade magnets.
...
In \secref{magnet-forces} when deriving the equations for the forces between magnets, it was assumed that each magnet had a constant and uniform magnetisation.
...
For example, the magnetic field of a grade \maggrade{35} cylindrical magnet with diameter \SI{100}{mm} and thickness \SI{30}{mm} was compared against theoretical calculations (performed with \ANSYS/) \cite{bruno2011-honoursthesis}.
While the comparison across axial position (\figref*{gauss-vertical}) only shows that the measured magnetic field is lower than expected, the lateral position results (\figref*{gauss-radial}) indicates why: the magnetic field is lower than expected closer to the centre of the magnet.
...
\caption[
  ...
  Magnetic flux density measurements of an \maggrade{35} \diameter\SI{100x30}{mm} cylindrical permanent magnet.
...
An example of this cross-coupling is shown in \figref{coupling-oscillations} for a
...
\begin{figure}
  ...
  \subbottom[Displacement trace.]{\psfragfig{\jobname/coupling-oscillations}}
...
\begin{figure}
   ...
   \caption
     [Individual and total vertical forces on the combined spring.]
     {Individual and total vertical forces on the combined spring for displacement in the $\az$ direction.
...
\newcommand\isoforces[3]{%
  \begin{figure}[p]
    \begin{wide}
      \vspace*{-0.5cm}
      \centerline{
      \subfloat[$\ax$ forces.\figlabel{iso-#1-x}]{%
        \psfragfig
          {PhD/Simulations/Single_magnets/all_springs/frag/#1-xforce}%
      }\hspace*{-0.5cm}%
      \subfloat[$\ay$ forces.\figlabel{iso-#1-y}]{%
        \psfragfig
          {PhD/Simulations/Single_magnets/all_springs/frag/#1-yforce}%
      }\hspace*{-0.5cm}%
      \subfloat[$\az$ forces.\figlabel{iso-#1-z}]{%
        \psfragfig
          {PhD/Simulations/Single_magnets/all_springs/frag/#1-zforce}%
      }%
      }%
    \end{wide}
    \caption[#2.]{#2 (\secref*{#3}).}
    \figlabel{#1-forces}
  \end{figure}
}

\isoforces{vattr}{Isoforces of the unstable vertical spring}{unstable-spring}
\isoforces{vrepl}{Isoforces of the stable vertical spring}{spring-repl}
\isoforces{z}{Isoforces of the \qzs/ spring}{qzs-basics}
\isoforces{h}{Isoforces of the horizontal spring}{hspring}
\isoforces{hv}{Isoforces of the combination spring}{spring-hv}
\isoforces{zhv}{Isoforces of the combination \qzs/ spring}{spring-zhv}
...
\begin{figure}
  ...
  \lofcaption{
    ...
    Plots are labelled in terms of the ratio $\leverArm_1/\leverArm_2$ between lever arm of the outer stabilising magnets and the inner strong magnets.
  }
...
\subfloat[With rotation shown in black; the unrotated position, as in (a), is shown in light gray.]
...
\subfloat[Relative vertical displacement $\mbqvdisp-\mbqvdisp_0$.]
  {\psfragfig{magcode/examples/oblique/fig/mbq-dyn-yr-y}}\qquad
\subfloat[Rotation $\mbqrotz$.]
  {\psfragfig{magcode/examples/oblique/fig/mbq-dyn-yr-r}}
\end{wide}
\caption
...
\subfloat[Horizontal displacement $\mbqhdisp$.]
  {\psfragfig{magcode/examples/oblique/fig/mbq-dyn-xyr-x}}\quad
\subfloat[Relative vert.\ displacement $\mbqvdisp-\mbqvdisp_0$.]
  {\psfragfig{magcode/examples/oblique/fig/mbq-dyn-xyr-y}}\quad
\subfloat[Rotation $\mbqrotz$.]
  {\psfragfig{magcode/examples/oblique/fig/mbq-dyn-xyr-r}}
\end{wide}
\caption
...
To explore the possibilities raised by the theoretical analysis of the inclined magnetic spring (\secref*{oblique}), an honours project co-supervised by the author was undertaken in 2011 to design and build a prototype based on these ideas \cite{frizenschaf2011-honoursthesis,frizenschaf2011-acoustics2011}.
...
\hfil
\subbottom
  [Damping ratio, shown with a line of best fit.\figlabel{oblique-damping}]
...
This peak can be reduced with active vibration control using skyhook damping (\secref*{skyhook-intro}).

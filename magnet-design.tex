...
While the comparison across axial displacement (\figref{gauss-axial}) only shows that the measured magnetic field is lower than expected, the lateral displacement results (\figref{gauss-lateral}) indicates why: the magnetic field is lower than expected closer to the centre of the magnet.

\begin{figure}
\begin{wide}
\begin{subfigure}[0.45]
\includegraphics{PhD/Figures/Magnets/gauss-test}
\caption{Axial displacement down the centreline.}
\figlabel{gauss-axial}
\end{subfigure}\hfil
\begin{subfigure}
\fbox{\parbox{5cm}{THOSE PROSTHETICS FOOLS DIDN'T PUT THIS GRAPH IN THEIR REPORT}}
\caption{Lateral displacement with a certain offset.}
\figlabel{gauss-lateral}
\end{subfigure}
\lofcaption{
...
\begin{figure}
  \includegraphics{PhD/Figures/Magnets/vspring}
...
\lofcaption{Schematic of a replusive magnetic spring.}{ The magnets have square facing sides and extend a distance $\faceCub$ in the direction toward the reader. When $\displ=0$ the magnet faces are touching.}
...
The force is calculated using \eqref{simpl4} with force \vs\ magnet ratio shown in \figref{magratio-cyl}.
...
\end{wide}
\lofcaption{Force between two magnets as a function of magnet ratio for a set of fixed displacements from $\SIrange{1}{10}{mm}$.}{ Dots mark the positions of maximum force.}
\end{figure}
...
The stiffness for this system was calculated as around \SI{-200}{N/m} and the displacement feedback gain was chosen as $\gainDisp=250$ to overcome the instability due to this.

\begin{figure}
  \psfragfig{\phdpath Simulations/Coupling/fig/coupling-oscillations}
  \lofcaption{Oscillations over time of the displacement of a suspended magnet that is excited through horizontal vibrations of the base magnet.}{ The horizontal vibration couples to the vertical forces.}
...
\section{Taxonomy of simple magnetic springs}
...
Six spring configurations are examined in \secref{isoforces} with \figrangeref{vattr-forces}{zhv-forces} showing for the different designs their contours of force \vs\ displacement in each direction.
...
Stability criteria: $K_z<0,K_x=K_y=-\half K_z>0$.
...
attraction.
This arrangement is discussed in the literature review in the context of active control methods (\secref{levitation-control}).
Although it is unstable in only a single \dof/, it is inconvenient to bear loads with a negative stiffness spring.


\subsection{Stable vertical}
...
Here, there is a positive spring stiffness in the vertical, load bearing direction.
However, it experiences multiple degrees of instability in the horizontal directions, which adds complication its stabilisation.

\subsection{\QZS/}
\seclabel{qzs-basics}

Stability criteria: $K_x=K_y=K_z=0$.

First introduced by \textcite{nijsse2001}, the \qzs/ spring, \figref{zero-stiffness-spring}, uses equal and opposite magnet pairs to create a force--displacement relationship that has a region of zero slope, shown in \figref{zspring-vary-gap} for half-inch magnet cubes.
With a positive vertical stiffness in series with a negative stiffness, the local minimum in the force/displacement curve creates a marginally stable point of \qzs/.
While the \qzs/ magnetic spring cannot be operated at its centred position of zero stiffness (as it has only marginal stability there), the upper attractive magnet can be considered to simply \emph{reduce} the natural frequency of the system without driving it to zero.
This improves vibration isolation characteristics without altering the stability of
the system (given bounded displacements), and this system is analysed in detail in \secref{qzs}..
This is the configuration used by \textcite{challa2008} in their tunable vibration energy harvesting device.

\begin{figure}
  \includegraphics{PhD/Figures/Systems/zero-stiffness-spring}
  \caption{`Zero stiffness' spring schematic.}
  \figlabel{zero-stiffness-spring}
\end{figure}

\begin{figure}
  \includegraphics{PhD/Simulations/Single_magnets/z_spring/eps/zspring-vary-gap}
  \caption{`Zero stiffness' vertical spring forces \vs\ vertical displacement.}
...
Stability criteria: $K_x<0,K_y=K_z=-\half K_x>0$.
...
\begin{figure}
  \begin{subfigure}
    \includegraphics{PhD/Figures/Magnets/hspringa}
    \caption{At rest, neglecting the magnet weight.}
    \figlabel{hspringa}
  \end{subfigure}
  \hfil
  \begin{subfigure}
    \includegraphics{PhD/Figures/Magnets/hspringb}
    \caption{After displacement.}
    \figlabel{hspringb}
  \end{subfigure}
  \caption[`Horizontal' attraction magnetic spring.]{A `horizontal' spring with \emph{attracting}
magnets to create vertical stiffness, as shown by the reaction forces after
displacement.}
  \figlabel{hspring}
\end{figure}

The force--displacement behaviour for the horizontal is a little more complex than for the vertical springs.
...
Beyond this, the force will start \emph{decreasing} as the floating magnet becomes further enough away from the fixed magnets — at which point the spring stiffness turns negative and the floating magnet will be unstable, falling to the ground under the influence of gravity.
...
These behaviours are shown in \figref{h+v-forces-v}, which also includes the force curve of the vertical spring for comparison.
(Magnets for the simulation were \SI{20}{mm} cubes with \SI{1}{T} magnetisation.)
The inflection point where the horizontal spring force reaches its peak can also considered to be a \qzs/ point; careful tuning of such a spring could yield similar advantages to the vertical \qzs/ spring introduced in \secref{qzs-basics}.
...
\begin{figure}
  ...
  \caption[Spring forces of the vertical \& horizontal springs.]{
    Spring forces of the vertical (dashed) and horizontal (solid) springs with cube magnets of size $m=\SI{20}{mm}$. The horizontal spring has a magnet face gap of $l/m\in\{0.1,0.5,1\}$.
  }
  \figlabel{h+v-forces-v}
\end{figure}


\subsection{Double stable}

Stability criteria: $K_z>0, K_x>0, K_y<0$.

This horizontal spring can be augmented with the stable vertical spring in order to increase its load bearing capacity.
\Figref{hvspring} shows how the vertical repulsion and horizontal springs may be combined.
There are now three fixed magnets: one below, which provides the majority of the stiffness of the spring, and two aside, for stabilising one \dof/ in the out-of-plane direction.
An alternative double-stable spring is discussed in \secref{choi-spring}.

\begin{figure}
   \includegraphics{PhD/Figures/Magnets/hvspring}
   \caption[Combination vertical/horizontal magnetic spring.]{Combination
magnetic spring, with the
strength of the vertical spring and the stability of the horizontal spring.}
   \figlabel{hvspring}
\end{figure}


\subsubsection{Combined \qzs/ spring}

Stability criteria: $K_z=0,K_y=-K_x>0$.

Finally, the horizontal and \qzs/ springs combined, shown in \figref{zhv-spring}, can produce a system approaching zero stiffness in the vertical direction, a small positive stiffness in one horizontal direction, and an equal negative stiffness in the other.
A demonstration of the force--displacement curves is shown in \figref{zhvspring-move,zhvspring-move-z}, for a system with half-inch cubes and a single magnet dimension gap between the floating magnet and all fixed magnets.
The self-weight of the system is taken to be \SI{5}{N} in order to include some effect of gravity compensation on the device.
The addition of the horizontal magnets produces an asymmetry in the total vertical force characteristic of the system, yet it still retains an inflection point at which \qzs/ can be realised.

\begin{figure}
   \includegraphics{PhD/Figures/Systems/zhv-spring}
   \lofcaption{Co-ordinate system of the `ZHV' spring.}
...
\begin{figure}
   \begin{wide}
   \subbottom[\x\ direction.\figlabel{zhvspring-move-x}]
...
\begin{figure}
   \centering
   \includegraphics{PhD/Simulations/Single_magnets/5_magnet_spring/eps/zhvspring-move-z}
   \lofcaption{Individual and combined vertical forces on the ZHV spring for
...
As the magnetic springs become composed of more elements, analysing their force--displacement characteristics is quite challenging.
As the \threeD/ forces vary with displacement in three \dof/, it isn't possible to directly visualise the behaviour these systems will have under arbitrary displacement.
An attempt to visualising the complex force profile these devices exhibit is shown in \figrangeref{vattr-forces}{zhv-forces}, in which representative force component contours are plotted against displacement in each direction separately; these contours are termed `isoforces' as they trace contours of equal force.

The simple magnetic systems of a pair of magnets in either attraction or repulsion will be treated first as their qualitative analysis is straightforward; consider their isoforces shown in \figref{vattr-forces,vrepl-forces}, \resp.
Each figure shows three plots which correspond to each component of force: $F_x$, $F_y$, $F_z$.
The plots are three dimensional, and contours represent the magnitude of the respective force component under three dimensional displacement.
...
Contrariwise, \figref{iso-vrepl-z} shows an increase in vertical force with negative displacement.
The colour scale goes through the spectrum from orange to purple as the forces increase.
For symmetrical cases like the \x- and \y-direction forces, this makes pale green-blue the colour of zero force, with orange/purple the negative and positive extremes, respectively.
...
The vertical \qzs/ spring retains this simplicity of representation in \figref{z-forces}, noting particularly the `switching-stability' horizontal behaviour with vertical displacement.
...
\Figref{h-forces,hv-forces,zhv-forces} involve the horizontal spring in combination with other arrangements and the superposition of the isoforces becomes increasingly complex.
In conclusion, combining simple magnets with a variety of geometries can lead to a wide selection of magnet spring types.
Only a preliminary analysis of these has been conducted here; in \secref{oblique} a different spring type using inclined magnets is covered in detail, and later in \secref{qzs} the design characteristics of the zero stiffness spring is examined.

\newcommand\isoforces[2]{%
  \begin{figure}[p]
    \begin{wide}
      \subfloat[$x$-forces.\figlabel{iso-#1-x}]{%
        \grf[width=0.3\linewidth]
          {Simulations/Single_magnets/all_springs/eps_nice/#1-xforce}
      }\hfil
      \subfloat[$y$-forces.\figlabel{iso-#1-y}]{%
        \grf[width=0.3\linewidth]
          {Simulations/Single_magnets/all_springs/eps_nice/#1-yforce}
      }\hfil
      \subfloat[$z$-forces.\figlabel{iso-#1-z}]{%
        \grf[width=0.3\linewidth]
          {Simulations/Single_magnets/all_springs/eps_nice/#1-zforce}
      }
    \end{wide}
    \caption{#2}
    \figlabel{#1-forces}
...
\isoforces{vattr}{Isoforces of the attracting vertical spring.}
\isoforces{vrepl}{Isoforces of the repelling vertical spring.}
\isoforces{z}{Isoforces of the zero stiffness spring.}
\isoforces{h}{Isoforces of the horizontal spring.}
\isoforces{hv}{Isoforces of the combination spring.}
\isoforces{zhv}{Isoforces of the combination zero stiffness spring.}
...
It is interesting as it has a single unstable degree of freedom.
...
\begin{figure}
  \begin{subfigure}
    \includegraphics{PhD/Figures/Bearings/delamare-forces}
    \caption{
  ...
  \end{subfigure}
  \begin{subfigure}
    \includegraphics{PhD/Figures/Bearings/delamare-moments}
    \caption{
...
\begin{figure}
  ...
  \caption
  [
    Moment of the magnet spring as it rotates around the $x$ and $z$ axes with a varying spring depth.
  ]
  {
...
To conclude, there are a plurality of magnet designs that can take advantage of mechanical level arms to stabilise rotational degrees of freedom even if low-order magnet systems exhibit rotational instability.
...
\Figref{mbq-wvf-angle} shows the natural frequency versus load curve for a magnet gap ratio of zero.
Increasing the magnet gap changes the force and stiffness relationships of the spring; \Figref{mbq-wvf-angle-2} shows the same plot with a magnet gap ratio of \num{0.25}.
The difference in the shape of the curves is not great, but \Figref {mbq-wvf-angle-2} shows that a greater magnet gap results in smaller load forces and a smaller range in load force.
...
To explore the possibilities raised by the theoretical analysis of the inclined magnetic spring (\secref{oblique}), an honours project was undertaken in 2011 to design and build a prototype based on these ideas \cite{frizenschaf2011-honoursthesis}.

...
\begin{figure}
  \psfragfig{\phdpath Figures/magnet-prices/mag-price}
...
In order to compare the force versus displacement characteristics for a range of magnet size ratios, we will normalise the forces by the force $F_s=F_m(V,1,x)$; that is, the force for a magnet size ratio $\gamma=1$. \Figref{magratios} shows the normalised force $\bar F=F/F_s$ as a function of displacement $x$ over a range of magnet size ratios $\gamma$. The figures are drawn as separate graphs in order to avoid overlap of the curves; size ratio $\gamma$ varies from $0$ to $0.4$ in \Figref{magratio1} and from $0.4$ to $0.8$ in \Figref{magratio2}. It can be seen from the two graphs that a magnet size ratio $\gamma$ of around $0.4$ produces the greatest forces; for values both less and greater than $0.4$, the normalised force curves decrease.

\begin{figure}[p]
\begin{wide}
\subfloat
  [Magnet ratios $\gamma=\{0.1,0.2,0.3,0.4\}$
   \figlabel{magratio1}]
  {\psfragfig{magcode/examples/fig/magratio1}}
\subfloat
  [Magnet ratios $\gamma=\{0.4,0.5,0.6,0.7,0.8\}$
   \figlabel{magratio2}]
  {\psfragfig{magcode/examples/fig/magratio2}}
\end{wide}
\caption{
  Normalised force $\bar F=F/F_s$ as a function of displacement $x$ for two sets of magnet ratios to avoid overlap of the graphs.
}
\figlabel{magratios}
\end{figure}
